...
void normal_child_inference(fs::path model_path, bool local=false)
{
        ...
        if(local) accl = new  MX::Runtime::MxAccl;
        else accl = new  MX::Runtime::MxAccl(true);
        int dfp_id = -1;
        try {
...
TEST(accl_multiprocess_connect,connect_dfp_same_dfp){
...
TEST(accl_multiprocess_connect,connect_dfp_different_dfp){
...
TEST(accl_multiprocess_connect,connect_dfp_different_after_exit){
    fs::path model_path = samples_path/"models"/"CenterNet"/"centernet_onnx.dfp";
    pid_t child_pid_list[2];
...
TEST(accl_multiprocess_connect,connect_dfp_remote_after_local){
    fs::path model_path = samples_path/"models"/"CenterNet"/"centernet_onnx.dfp";
    pid_t child_pid_list[2];
    pid_t pid = fork();
    if (pid < 0) {
...
TEST(accl_multiprocess_connect,connect_dfp_local_after_remote){
    fs::path model_path = samples_path/"models"/"CenterNet"/"centernet_onnx.dfp";
    pid_t child_pid_list[2];
...
TEST(accl_multiprocess_test,connect_dfp_remote_and_local){
    fs::path model_path = samples_path/"models"/"CenterNet"/"centernet_onnx.dfp";
    pid_t child_pid_list[2];
    for (int i = 0; i < 2; i++) {
        std::this_thread::sleep_for(100ms);
        ...
        if (pid < 0) {
            ...
            MX::Runtime::MxAccl* accl;
            if(i==0) accl = new  MX::Runtime::MxAccl(true);
            else accl = new  MX::Runtime::MxAccl;
...
TEST(accl_multiprocess_test,connect_dfp_local_and_remote){
    fs::path model_path = samples_path/"models"/"CenterNet"/"centernet_onnx.dfp";
    pid_t child_pid_list[2];
    for (int i = 0; i < 2; i++) {
        std::this_thread::sleep_for(100ms);
        ...
        if (pid < 0) {
            ...
            MX::Runtime::MxAccl* accl;
            if(i==1) accl = new  MX::Runtime::MxAccl(true);
            else accl = new  MX::Runtime::MxAccl;
