...
class PrePost{
    public:
        MEMX_API_EXPORT virtual void runinference(std::vector<MX::Types::FeatureMap<float>*> input, std::vector<MX::Types::FeatureMap<float>*> output) = 0;
        MEMX_API_EXPORT virtual void runinference(std::vector<MX::Types::FeatureMap<uint8_t>*>, std::vector<MX::Types::FeatureMap<uint8_t>*>){
...
MEMX_API_EXPORT PrePost* createObject(std::string soFileName, std::string functionName, std::string model_path, const std::vector<size_t>& out_sizes, PluginType type);
