...
#ifndef MXASIO_SPAWN_HPP
#define MXASIO_SPAWN_HPP
...
#include "mxasio/detail/exception.hpp"
...
public:
   ...
   * mxasio::cancellation_state object <tt>S</tt>, constructed as
   ...
   * Let <tt>P</tt> be the cancellation slot associated with the current
   * coroutine's @ref spawn completion handler. Assigns a new
   * mxasio::cancellation_state object <tt>S</tt>, constructed as <tt>S(P,
   ...
   * } @endcode
   */
  basic_yield_context operator[](mxasio::error_code& ec) const
  {
 ...
 *
 * @code mxasio::spawn(my_strand, do_echo, mxasio::detached);
 *
 * // ...
 *
 * void do_echo(mxasio::yield_context yield)
 * {
 *   try
 *   {
 *     char data[128];
 ...
 *           mxasio::buffer(data), yield);
 *
 *       mxasio::async_write(my_socket,
 *           mxasio::buffer(data, length), yield);
 *     }
 *   }
 ...
 */
template <typename Function>
void spawn(const mxasio::io_context::strand& s, Function&& function,
    const boost::coroutines::attributes& attributes
      = boost::coroutines::attributes());
...
#include "mxasio/impl/spawn.hpp"

#endif // MXASIO_SPAWN_HPP
