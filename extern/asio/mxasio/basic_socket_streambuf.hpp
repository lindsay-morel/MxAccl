...
#ifndef MXASIO_BASIC_SOCKET_STREAMBUF_HPP
#define MXASIO_BASIC_SOCKET_STREAMBUF_HPP
...
#include "mxasio/io_context.hpp"
  ...
  && defined(MXASIO_USE_BOOST_DATE_TIME_FOR_SOCKET_IOSTREAM)
...
# endif // !defined(MXASIO_NO_DEPRECATED)
   ...
   * buffer.
   */
  const mxasio::error_code& error() const
  {
    return ec_;
  }
   ...
   * buffer.
   */
  const mxasio::error_code& puberror() const
  {
    return error();
  }
   ...
   * mxasio::error::operation_aborted.
   *
...
protected:
  int_type underflow()
  {
    ...
    ec_ = mxasio::error::operation_not_supported;
    ...
    for (;;)
    {
      ...
      detail::buffer_sequence_adapter<mutable_buffer, mutable_buffer>
        bufs(mxasio::buffer(get_buffer_) + putback_max);
      ...
      if (bytes == 0)
      {
        ec_ = mxasio::error::eof;
        return traits_type::eof();
      }

      // Operation failed.
      if (ec_ != mxasio::error::would_block
          && ec_ != mxasio::error::try_again)
        return traits_type::eof();
  ...
  int_type overflow(int_type c)
  {
    ...
    if (put_buffer_.empty())
    {
      ...
      output_buffer = mxasio::buffer(&ch, sizeof(char_type));
    }
    else
    {
      output_buffer = mxasio::buffer(pbase(),
...
private:
  ...
  void connect_to_endpoints(EndpointIterator begin, EndpointIterator end)
  {
    ...
    for (EndpointIterator i = begin; i != end; ++i)
    {
      ...
      // Operation failed.
      if (ec_ != mxasio::error::in_progress
          && ec_ != mxasio::error::would_block)
        continue;
      ...
      ec_ = mxasio::error_code(connect_error,
...
#endif // MXASIO_BASIC_SOCKET_STREAMBUF_HPP
