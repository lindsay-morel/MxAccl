...
#ifndef MXASIO_BASIC_WRITABLE_PIPE_HPP
#define MXASIO_BASIC_WRITABLE_PIPE_HPP
...
public:
   ...
   */
  MXASIO_SYNC_OP_VOID assign(const native_handle_type& native_pipe,
      mxasio::error_code& ec)
  {
    impl_.get_service().assign(impl_.get_implementation(), native_pipe, ec);
   ...
   * @code
   * pipe.write_some(mxasio::buffer(data, size));
   * @endcode
   ...
   * To write a single data buffer use the @ref buffer function as follows:
   * @code
   * pipe.async_write_some(mxasio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
...
#endif // MXASIO_BASIC_WRITABLE_PIPE_HPP
