...
#ifndef MXASIO_CONNECT_HPP
#define MXASIO_CONNECT_HPP
...
/**
 * @defgroup connect mxasio::connect
 *
 ...
 *
 * @throws mxasio::system_error Thrown on failure. If the sequence is
 * empty, the associated @c error_code is mxasio::error::not_found.
 ...
 * mxasio::connect(s, r.resolve(q)); @endcode
 */
 ...
 * empty, set to mxasio::error::not_found. Otherwise, contains the error
 ...
 * mxasio::connect(s, r.resolve(q), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
...
template <typename Protocol, typename Executor, typename EndpointSequence>
typename Protocol::endpoint connect(basic_socket<Protocol, Executor>& s,
    const EndpointSequence& endpoints, mxasio::error_code& ec,
    constraint_t<is_endpoint_sequence<EndpointSequence>::value> = 0);
 ...
 * iterator types such as @c mxasio::ip::tcp::resolver::iterator.
 */
...
Iterator connect(basic_socket<Protocol, Executor>& s,
    Iterator begin, mxasio::error_code& ec,
    constraint_t<!is_endpoint_sequence<Iterator>::value> = 0);
 ...
 * tcp::socket s(my_context);
 * mxasio::connect(s, e.begin(), e.end()); @endcode
 */
template <typename Protocol, typename Executor, typename Iterator>
Iterator connect(basic_socket<Protocol, Executor>& s,
 ...
 * tcp::socket s(my_context);
 * mxasio::error_code ec;
 * mxasio::connect(s, e.begin(), e.end(), ec);
 * if (ec)
 * {
 *   // An error occurred.
 * } @endcode
...
template <typename Protocol, typename Executor, typename Iterator>
Iterator connect(basic_socket<Protocol, Executor>& s,
    Iterator begin, Iterator end, mxasio::error_code& ec);
 ...
 *   }
 * }; @endcode
 * It would be used with the mxasio::connect function as follows:
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 * tcp::socket s(my_context);
 * tcp::endpoint e = mxasio::connect(s,
...
template <typename Protocol, typename Executor,
    typename Iterator, typename ConnectCondition>
Iterator connect(basic_socket<Protocol, Executor>& s, Iterator begin,
    ConnectCondition connect_condition, mxasio::error_code& ec,
    constraint_t<!is_endpoint_sequence<Iterator>::value> = 0);
#endif // !defined(MXASIO_NO_DEPRECATED)
 ...
 * tcp::resolver::results_type e = r.resolve(q);
 * tcp::socket s(my_context);
 * tcp::resolver::results_type::iterator i = mxasio::connect(
...
/**
 * @defgroup async_connect mxasio::async_connect
 *
 ...
 *   // mxasio::error::not_found. Otherwise, contains the
 ...
 * @code void(mxasio::error_code, typename Protocol::endpoint) @endcode
 *
 * @par Example
 * @code tcp::resolver r(my_context);
 * tcp::resolver::query q("host", "service");
 ...
 *   if (!ec)
 *   {
 *     mxasio::async_connect(s, results, connect_handler);
 *   }
 * }
 *
 * // ...
...
auto async_connect(basic_socket<Protocol, Executor>& s,
  ...
  -> decltype(
    async_initiate<RangeConnectToken,
      void (mxasio::error_code, typename Protocol::endpoint)>(
 ...
 *
 * @par Completion Signature
 * @code void(mxasio::error_code, Iterator) @endcode
 *
 * @note This overload assumes that a default constructed object of type @c
 * Iterator represents the end of the sequence. This is a valid assumption for
 * iterator types such as @c mxasio::ip::tcp::resolver::iterator.
...
auto async_connect(basic_socket<Protocol, Executor>& s, Iterator begin,
  ...
  -> decltype(
    async_initiate<IteratorConnectToken,
      void (mxasio::error_code, Iterator)>(
 ...
 *   if (!ec)
 *   {
 *     mxasio::async_connect(s, results,
...
#include "mxasio/impl/connect.hpp"
