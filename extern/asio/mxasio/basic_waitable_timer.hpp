...
#ifndef MXASIO_BASIC_WAITABLE_TIMER_HPP
#define MXASIO_BASIC_WAITABLE_TIMER_HPP
...
#include "mxasio/detail/chrono_time_traits.hpp"
#include "mxasio/detail/deadline_timer_service.hpp"
...
template <typename Clock,
    typename WaitTraits = mxasio::wait_traits<Clock>,
 ...
 *
 * Most applications will use one of the mxasio::steady_timer,
 * mxasio::system_timer or mxasio::high_resolution_timer typedefs.
 *
 ...
 * mxasio::steady_timer timer(my_context);
 *
 ...
 * @code
 * void handler(const mxasio::error_code& error)
 * {
 *   if (!error)
 *   {
 *     // Timer expired.
 ...
 * }
 *
 * void on_timeout(const mxasio::error_code& e)
 * {
 *   if (e != mxasio::error::operation_aborted)
 *   {
 ...
 * @endcode
 *
 * @li The mxasio::basic_waitable_timer::expires_after() function
 * cancels any pending asynchronous waits, and returns the number of
 * asynchronous waits that were cancelled. If it returns 0 then you were too
 * late and the wait handler has already been executed, or will soon be
 * executed. If it returns 1 then the wait handler was successfully cancelled.
 *
 * @li If a wait handler is cancelled, the mxasio::error_code passed to
 * it contains the value mxasio::error::operation_aborted.
 */
...
public:
    ...
    : impl_(0, ex)
  {
    ...
    mxasio::detail::throw_error(ec, "expires_at");
  }
    ...
    : impl_(0, ex)
  {
    ...
    mxasio::detail::throw_error(ec, "expires_after");
  }
   ...
   * be invoked with the mxasio::error::operation_aborted error code.
   *
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel(mxasio::error_code& ec)
  {
   ...
   * mxasio::error::operation_aborted error code.
   *
   * Cancelling the timer does not change the expiry time.
   *
  ...
  std::size_t cancel_one()
  {
    ...
    mxasio::detail::throw_error(ec, "cancel_one");
    return s;
  }
   ...
   * error code that indicates the successful completion of the wait operation.
   */
  std::size_t cancel_one(mxasio::error_code& ec)
  {
  ...
  std::size_t expires_from_now(const duration& expiry_time)
  {
    ...
    std::size_t s = impl_.get_service().expires_from_now(
        impl_.get_implementation(), expiry_time, ec);
    mxasio::detail::throw_error(ec, "expires_from_now");
    return s;
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  void wait(mxasio::error_code& ec)
  {
    impl_.get_service().wait(impl_.get_implementation(), ec);
  }
   ...
   * code mxasio::error::operation_aborted.
   *
...
#endif // MXASIO_BASIC_WAITABLE_TIMER_HPP
