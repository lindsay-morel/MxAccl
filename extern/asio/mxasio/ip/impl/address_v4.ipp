...
#ifndef MXASIO_IP_IMPL_ADDRESS_V4_IPP
#define MXASIO_IP_IMPL_ADDRESS_V4_IPP
...
address_v4::address_v4(address_v4::uint_type addr)
{
  ...
  addr_.s_addr = mxasio::detail::socket_ops::host_to_network_long(
      static_cast<mxasio::detail::u_long_type>(addr));
}
...
{
  return mxasio::detail::socket_ops::network_to_host_long(addr_.s_addr);
}
...
std::string address_v4::to_string() const
{
  ...
  char addr_str[mxasio::detail::max_addr_v4_str_len];
  const char* addr =
    mxasio::detail::socket_ops::inet_ntop(
        MXASIO_OS_DEF(AF_INET), &addr_, addr_str,
        mxasio::detail::max_addr_v4_str_len, 0, ec);
  if (addr == 0)
...
#if !defined(MXASIO_NO_DEPRECATED)
std::string address_v4::to_string(mxasio::error_code& ec) const
{
  char addr_str[mxasio::detail::max_addr_v4_str_len];
  const char* addr =
    mxasio::detail::socket_ops::inet_ntop(
    ...
    mxasio::error_code& ec) noexcept
{
  ...
  if (mxasio::detail::socket_ops::inet_pton(
        MXASIO_OS_DEF(AF_INET), str, &bytes, 0, ec) <= 0)
    return address_v4();
...
#endif // MXASIO_IP_IMPL_ADDRESS_V4_IPP
