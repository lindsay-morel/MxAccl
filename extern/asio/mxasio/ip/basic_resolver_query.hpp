...
#ifndef MXASIO_IP_BASIC_RESOLVER_QUERY_HPP
#define MXASIO_IP_BASIC_RESOLVER_QUERY_HPP
...
/**
 * The mxasio::ip::basic_resolver_query class template describes a query
...
public:
      ...
      service_name_(service)
  {
    typename InternetProtocol::endpoint endpoint;
    hints_.ai_flags = static_cast<int>(resolve_flags);
    hints_.ai_family = MXASIO_OS_DEF(AF_UNSPEC);
    hints_.ai_socktype = endpoint.protocol().type();
    hints_.ai_protocol = endpoint.protocol().protocol();
    hints_.ai_addrlen = 0;
    hints_.ai_canonname = 0;
  ...
  const mxasio::detail::addrinfo_type& hints() const
  {
    return hints_;
  }
...
private:
  mxasio::detail::addrinfo_type hints_;
...
#endif // MXASIO_IP_BASIC_RESOLVER_QUERY_HPP
