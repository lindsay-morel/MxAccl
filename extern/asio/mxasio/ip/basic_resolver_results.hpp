...
#ifndef MXASIO_IP_BASIC_RESOLVER_RESULTS_HPP
#define MXASIO_IP_BASIC_RESOLVER_RESULTS_HPP
...
/**
 * The mxasio::ip::basic_resolver_results class template is used to define
...
public:
  ...
  static basic_resolver_results create(
      mxasio::detail::addrinfo_type* address_info,
      const std::string& host_name, const std::string& service_name)
  {
    ...
    while (address_info)
    {
      if (address_info->ai_family == MXASIO_OS_DEF(AF_INET)
          || address_info->ai_family == MXASIO_OS_DEF(AF_INET6))
      {
...
# if defined(MXASIO_WINDOWS_RUNTIME)
  ...
  static basic_resolver_results create(
      ...
      const mxasio::detail::addrinfo_type& hints,
      const std::string& host_name, const std::string& service_name)
  {
    basic_resolver_results results;
    if (endpoints->Size)
    {
      ...
      for (unsigned int i = 0; i < endpoints->Size; ++i)
      {
        ...
        if (hints.ai_family == MXASIO_OS_DEF(AF_INET)
        ...
        results.values_->push_back(
            basic_resolver_entry<InternetProtocol>(
              typename InternetProtocol::endpoint(
                ip::make_address(
                  mxasio::detail::winrt_utils::string(
                    pair->RemoteHostName->CanonicalName)),
                mxasio::detail::winrt_utils::integer(
...
#endif // MXASIO_IP_BASIC_RESOLVER_RESULTS_HPP
