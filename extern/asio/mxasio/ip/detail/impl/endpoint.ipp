...
#ifndef MXASIO_IP_DETAIL_IMPL_ENDPOINT_IPP
#define MXASIO_IP_DETAIL_IMPL_ENDPOINT_IPP
...
endpoint::endpoint() noexcept
  : data_()
{
  data_.v4.sin_family = MXASIO_OS_DEF(AF_INET);
  data_.v4.sin_port = 0;
  data_.v4.sin_addr.s_addr = MXASIO_OS_DEF(INADDR_ANY);
}
  ...
  : data_()
{
  ...
  if (family == MXASIO_OS_DEF(AF_INET))
  {
    data_.v4.sin_family = MXASIO_OS_DEF(AF_INET);
    data_.v4.sin_port =
      mxasio::detail::socket_ops::host_to_network_short(port_num);
    data_.v4.sin_addr.s_addr = MXASIO_OS_DEF(INADDR_ANY);
  }
  else
  {
    data_.v6.sin6_family = MXASIO_OS_DEF(AF_INET6);
    data_.v6.sin6_port =
      mxasio::detail::socket_ops::host_to_network_short(port_num);
...
endpoint::endpoint(const mxasio::ip::address& addr,
  ...
  : data_()
{
  ...
  if (addr.is_v4())
  {
    ...
    data_.v4.sin_addr.s_addr =
      mxasio::detail::socket_ops::host_to_network_long(
        addr.to_v4().to_uint());
  }
  else
  {
    ...
    data_.v6.sin6_port =
      mxasio::detail::socket_ops::host_to_network_short(port_num);
    data_.v6.sin6_flowinfo = 0;
    mxasio::ip::address_v6 v6_addr = addr.to_v6();
    mxasio::ip::address_v6::bytes_type bytes = v6_addr.to_bytes();
    memcpy(data_.v6.sin6_addr.s6_addr, bytes.data(), 16);
    data_.v6.sin6_scope_id =
      static_cast<mxasio::detail::u_long_type>(
        v6_addr.scope_id());
  }
}
...
unsigned short endpoint::port() const noexcept
{
  if (is_v4())
  {
    return mxasio::detail::socket_ops::network_to_host_short(
        data_.v4.sin_port);
  }
  else
  {
...
mxasio::ip::address endpoint::address() const noexcept
{
  using namespace std; // For memcpy.
  if (is_v4())
  {
    return mxasio::ip::address_v4(
        mxasio::detail::socket_ops::network_to_host_long(
          data_.v4.sin_addr.s_addr));
  }
  else
  {
    mxasio::ip::address_v6::bytes_type bytes;
    memcpy(bytes.data(), data_.v6.sin6_addr.s6_addr, 16);
    return mxasio::ip::address_v6(bytes, data_.v6.sin6_scope_id);
  }
}

void endpoint::address(const mxasio::ip::address& addr) noexcept
{
...
#endif // MXASIO_IP_DETAIL_IMPL_ENDPOINT_IPP
