...
#ifndef MXASIO_IP_BASIC_RESOLVER_HPP
#define MXASIO_IP_BASIC_RESOLVER_HPP
...
#include "mxasio/detail/winrt_resolver_service.hpp"
#else
#include "mxasio/detail/resolver_service.hpp"
#endif
...
public:
   ...
   * will be invoked with the mxasio::error::operation_aborted error code.
   */
  void cancel()
  {
  ...
  results_type resolve(const query& q)
  {
    ...
    mxasio::detail::throw_error(ec, "resolve");
    return r;
  }
   ...
   */
  results_type resolve(const query& q, mxasio::error_code& ec)
  {
   ...
   */
  results_type resolve(MXASIO_STRING_VIEW_PARAM host,
      MXASIO_STRING_VIEW_PARAM service)
  {
   ...
   */
  results_type resolve(MXASIO_STRING_VIEW_PARAM host,
      MXASIO_STRING_VIEW_PARAM service, mxasio::error_code& ec)
  {
   ...
   */
  results_type resolve(MXASIO_STRING_VIEW_PARAM host,
      MXASIO_STRING_VIEW_PARAM service, resolver_base::flags resolve_flags)
  {
    mxasio::error_code ec;
  ...
  results_type resolve(const protocol_type& protocol,
      MXASIO_STRING_VIEW_PARAM host, MXASIO_STRING_VIEW_PARAM service)
  {
  ...
  results_type resolve(const protocol_type& protocol,
      MXASIO_STRING_VIEW_PARAM host, MXASIO_STRING_VIEW_PARAM service,
      resolver_base::flags resolve_flags, mxasio::error_code& ec)
  {
    basic_resolver_query<protocol_type> q(
        protocol, static_cast<std::string>(host),
        static_cast<std::string>(service), resolve_flags);
   ...
   * @code void(mxasio::error_code, results_type) @endcode
   */
  template <
  ...
  auto async_resolve(const query& q,
    ...
    -> decltype(
      mxasio::async_initiate<ResolveToken,
        void (mxasio::error_code, results_type)>(
          declval<initiate_async_resolve>(), token, q))
  {
    return mxasio::async_initiate<ResolveToken,
      void (mxasio::error_code, results_type)>(
  ...
  template <
      MXASIO_COMPLETION_TOKEN_FOR(void (mxasio::error_code,
        results_type)) ResolveToken = default_completion_token_t<executor_type>>
  auto async_resolve(MXASIO_STRING_VIEW_PARAM host,
      MXASIO_STRING_VIEW_PARAM service,
      ResolveToken&& token = default_completion_token_t<executor_type>())
    -> decltype(
      mxasio::async_initiate<ResolveToken,
   ...
   * function is guaranteed to return a non-empty range.
   */
  results_type resolve(const endpoint_type& e, mxasio::error_code& ec)
  {
...
private:
  ...
  public:
        ...
        const Query& q) const
    {
      ...
      MXASIO_RESOLVE_HANDLER_CHECK(
          ResolveHandler, handler, results_type) type_check;

      mxasio::detail::non_const_lvalue<ResolveHandler> handler2(handler);
...
# if defined(MXASIO_WINDOWS_RUNTIME)
  mxasio::detail::io_object_impl<
    mxasio::detail::winrt_resolver_service<InternetProtocol>,
    Executor> impl_;
# else
  mxasio::detail::io_object_impl<
    mxasio::detail::resolver_service<InternetProtocol>,
    Executor> impl_;
# endif
};
...
#endif // MXASIO_IP_BASIC_RESOLVER_HPP
