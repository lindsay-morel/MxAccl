...
#ifndef MXASIO_STRAND_HPP
#define MXASIO_STRAND_HPP
...
#include "mxasio/detail/strand_executor_service.hpp"
...
public:
   ...
   *
   * For example:
   * @code mxasio::strand<my_executor_type> ex = ...;
   * if (mxasio::query(ex, mxasio::execution::blocking)
   *       == mxasio::execution::blocking.never)
   *   ... @endcode
   */
   ...
   *
   * For example:
   * @code mxasio::strand<my_executor_type> ex1 = ...;
    ...
    noexcept(is_nothrow_require<const Executor&, Property>::value)
  {
    return strand<decay_t<require_result_t<const Executor&, Property>>>(
        mxasio::require(executor_, p), impl_);
  }
    ...
    noexcept(is_nothrow_prefer<const Executor&, Property>::value)
  {
    return strand<decay_t<prefer_result_t<const Executor&, Property>>>(
        mxasio::prefer(executor_, p), impl_);
  }
...
private:
      ...
      > = 0)
  {
    return use_service<detail::strand_executor_service>(
        mxasio::query(ex, execution::context)).create_implementation();
  }
  ...
  {
    return mxasio::query(executor_, property);
  }

  template <typename Property>
  execution::blocking_t query_helper(true_type, const Property& property) const
  {
    execution::blocking_t result = mxasio::query(executor_, property);
...
/** @defgroup make_strand mxasio::make_strand
 *
 * @brief The mxasio::make_strand function creates a @ref strand object for
...
# if defined(MXASIO_IO_CONTEXT_HPP)
#include "mxasio/io_context_strand.hpp"
# endif // defined(MXASIO_IO_CONTEXT_HPP)
#endif // !defined(MXASIO_NO_EXTENSIONS)

#endif // MXASIO_STRAND_HPP
