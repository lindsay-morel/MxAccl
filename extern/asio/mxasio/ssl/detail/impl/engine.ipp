...
#ifndef MXASIO_SSL_DETAIL_IMPL_ENGINE_IPP
#define MXASIO_SSL_DETAIL_IMPL_ENGINE_IPP
...
#include "mxasio/ssl/verify_context.hpp"
  ...
  : ssl_(::SSL_new(context))
{
  if (!ssl_)
  {
    ...
    mxasio::detail::throw_error(ec, "engine");
  }
...
mxasio::error_code engine::set_verify_mode(
...
mxasio::error_code engine::set_verify_depth(
...
mxasio::error_code engine::set_verify_callback(
...
{
  return perform((type == mxasio::ssl::stream_base::client)
      ? &engine::do_connect : &engine::do_accept, 0, 0, ec, 0);
}

engine::want engine::shutdown(mxasio::error_code& ec)
{
  return perform(&engine::do_shutdown, 0, 0, ec, 0);
}

engine::want engine::write(const mxasio::const_buffer& data,
...
engine::want engine::read(const mxasio::mutable_buffer& data,
    mxasio::error_code& ec, std::size_t& bytes_transferred)
{
  if (data.size() == 0)
  {
...
mxasio::mutable_buffer engine::get_output(
    const mxasio::mutable_buffer& data)
{
  int length = ::BIO_read(ext_bio_,
      data.data(), static_cast<int>(data.size()));

  return mxasio::buffer(data,
      length > 0 ? static_cast<std::size_t>(length) : 0);
}

mxasio::const_buffer engine::put_input(
...
const mxasio::error_code& engine::map_error_code(
    mxasio::error_code& ec) const
{
  ...
  {
    ec = mxasio::ssl::error::stream_truncated;
    return ec;
  }
...
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
mxasio::detail::static_mutex& engine::accept_mutex()
{
  static mxasio::detail::static_mutex mutex = MXASIO_STATIC_MUTEX_INIT;
  return mutex;
}
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
    ...
    std::size_t* bytes_transferred)
{
  ...
  {
    ec = mxasio::error_code(sys_error,
        mxasio::error::get_ssl_category());
  ...
  if (ssl_error == SSL_ERROR_SYSCALL)
  {
    if (sys_error == 0)
    {
      ec = mxasio::ssl::error::unspecified_system_error;
    }
    else
    {
      ec = mxasio::error_code(sys_error,
  ...
  {
    ec = mxasio::ssl::error::unexpected_result;
    return want_nothing;
  }
}
...
{
#if (OPENSSL_VERSION_NUMBER < 0x10000000L)
  mxasio::detail::static_mutex::scoped_lock lock(accept_mutex());
#endif // (OPENSSL_VERSION_NUMBER < 0x10000000L)
...
#endif // MXASIO_SSL_DETAIL_IMPL_ENGINE_IPP
