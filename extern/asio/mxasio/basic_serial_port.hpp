...
#ifndef MXASIO_BASIC_SERIAL_PORT_HPP
#define MXASIO_BASIC_SERIAL_PORT_HPP
...
#include "mxasio/detail/win_iocp_serial_port_service.hpp"
#else
...
public:
   ...
   */
  MXASIO_SYNC_OP_VOID open(const std::string& device,
      mxasio::error_code& ec)
  {
    impl_.get_service().open(impl_.get_implementation(), device, ec);
   ...
   */
  MXASIO_SYNC_OP_VOID assign(const native_handle_type& native_serial_port,
      mxasio::error_code& ec)
  {
    impl_.get_service().assign(impl_.get_implementation(),
        native_serial_port, ec);
  ...
  void send_break()
  {
    ...
    mxasio::detail::throw_error(ec, "send_break");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID send_break(mxasio::error_code& ec)
  {
    impl_.get_service().send_break(impl_.get_implementation(), ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * mxasio::serial_port_base::baud_rate @n
   * mxasio::serial_port_base::flow_control @n
   * mxasio::serial_port_base::parity @n
   * mxasio::serial_port_base::stop_bits @n
   * mxasio::serial_port_base::character_size
   */
   ...
   */
  template <typename SettableSerialPortOption>
  MXASIO_SYNC_OP_VOID set_option(const SettableSerialPortOption& option,
      mxasio::error_code& ec)
  {
    impl_.get_service().set_option(impl_.get_implementation(), option, ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
   ...
   */
  template <typename GettableSerialPortOption>
  MXASIO_SYNC_OP_VOID get_option(GettableSerialPortOption& option,
   ...
   *
   * @throws mxasio::system_error Thrown on failure. An error code of
   * mxasio::error::eof indicates that the connection was closed by the
   * peer.
   *
   ...
   * @code
   * basic_serial_port.write_some(mxasio::buffer(data, size));
   * @endcode
   ...
   *     mxasio::buffer(data, size), handler);
   * @endcode
   * See the @ref buffer documentation for information on writing multiple
   * buffers in one go, and how to use it with arrays, boost::array or
   * std::vector.
   ...
   * @code
   * basic_serial_port.read_some(mxasio::buffer(data, size));
   * @endcode
...
#endif // MXASIO_BASIC_SERIAL_PORT_HPP
