...
#ifndef MXASIO_BASIC_FILE_HPP
#define MXASIO_BASIC_FILE_HPP
...
#include "mxasio/detail/win_iocp_file_service.hpp"
#elif defined(MXASIO_HAS_IO_URING)
#include "mxasio/detail/io_uring_file_service.hpp"
#endif
...
#if !defined(MXASIO_BASIC_FILE_FWD_DECL)
#define MXASIO_BASIC_FILE_FWD_DECL

// Forward declaration with defaulted arguments.
template <typename Executor = any_io_executor>
class basic_file;

#endif // !defined(MXASIO_BASIC_FILE_FWD_DECL)
...
public:
   ...
   * @par Example
   * @code
   * mxasio::stream_file file(my_context);
   * file.open("/path/to/my/file", mxasio::stream_file::read_only);
   * @endcode
   */
   ...
   * mxasio::stream_file file(my_context);
   * mxasio::error_code ec;
   * file.open("/path/to/my/file", mxasio::stream_file::read_only, ec);
   * if (ec)
   * {
   *   // An error occurred.
   * }
   * @endcode
   */
  MXASIO_SYNC_OP_VOID open(const char* path,
      file_base::flags open_flags, mxasio::error_code& ec)
  {
    impl_.get_service().open(impl_.get_implementation(), path, open_flags, ec);
   ...
   * @endcode
   */
  MXASIO_SYNC_OP_VOID open(const std::string& path,
      file_base::flags open_flags, mxasio::error_code& ec)
  {
    impl_.get_service().open(impl_.get_implementation(),
        path.c_str(), open_flags, ec);
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID assign(const native_handle_type& native_file,
      mxasio::error_code& ec)
  {
    impl_.get_service().assign(
        impl_.get_implementation(), native_file, ec);
   ...
   * operations will be passed the mxasio::error::operation_aborted error.
  ...
  uint64_t size() const
  {
    ...
    mxasio::detail::throw_error(ec, "size");
    return s;
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  uint64_t size(mxasio::error_code& ec) const
  {
  ...
  void resize(uint64_t n)
  {
    ...
    mxasio::detail::throw_error(ec, "resize");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID resize(uint64_t n, mxasio::error_code& ec)
  {
    impl_.get_service().resize(impl_.get_implementation(), n, ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  }
  ...
  void sync_all()
  {
    ...
    mxasio::detail::throw_error(ec, "sync_all");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID sync_all(mxasio::error_code& ec)
  {
    impl_.get_service().sync_all(impl_.get_implementation(), ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  }
  ...
  void sync_data()
  {
    ...
    mxasio::detail::throw_error(ec, "sync_data");
  }
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID sync_data(mxasio::error_code& ec)
  {
    impl_.get_service().sync_data(impl_.get_implementation(), ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  }
