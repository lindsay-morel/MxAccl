...
//
#ifndef MXASIO_EXPERIMENTAL_IMPL_CORO_HPP
#define MXASIO_EXPERIMENTAL_IMPL_CORO_HPP
...
#include "mxasio/experimental/detail/coro_completion_handler.hpp"
...
template <>
struct coro_error<mxasio::error_code>
{
  static mxasio::error_code invalid()
  {
    return mxasio::error::fault;
  }

  static mxasio::error_code cancelled()
  {
    return mxasio::error::operation_aborted;
  }

  static mxasio::error_code interrupted()
  {
    return mxasio::error::interrupted;
  }

  static mxasio::error_code done()
  {
    return mxasio::error::broken_pipe;
  }
};
...
struct coro_error<std::exception_ptr>
{
  static std::exception_ptr invalid()
  {
    return std::make_exception_ptr(
        mxasio::system_error(
          coro_error<mxasio::error_code>::invalid()));
  }
  ...
  static std::exception_ptr cancelled()
  {
    return std::make_exception_ptr(
        mxasio::system_error(
          coro_error<mxasio::error_code>::cancelled()));
  }
  ...
  static std::exception_ptr interrupted()
  {
    return std::make_exception_ptr(
        mxasio::system_error(
          coro_error<mxasio::error_code>::interrupted()));
  }
  ...
  static std::exception_ptr done()
  {
    return std::make_exception_ptr(
        mxasio::system_error(
          coro_error<mxasio::error_code>::done()));
  }
};
      ...
      -> coroutine_handle<>
    {
      ...
      if constexpr (!coro_promise<Y, R, E, A>::is_noexcept)
      {
            ...
            && hp.cancel->throw_if_cancelled_)
        {
          mxasio::detail::throw_error(
              mxasio::error::operation_aborted, "coro-cancelled");
        }
      }
      ...
      else
      {
        ...
        struct cancel_handler
        {
          ...
          {
            mxasio::dispatch(e, [ct, st = st]() mutable
            {
              auto & [sig, state] = *st;
              sig.emit(ct);
            });
      ...
      typename coro_traits<Yield, Return, Executor>::return_type>
{
  ...
  using cancellation_slot_type = mxasio::cancellation_slot;
  ...
  auto await_transform(this_coro::cancellation_state_t) const
  {
    struct exec_helper
    {
      const mxasio::cancellation_state& value;
      ...
      mxasio::cancellation_state await_resume() const noexcept
      {
        return value;
      }
    };
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
      ...
      std::true_type /* result is void */)
  {
    ...
    {
      if (!the_coro)
        return mxasio::post(exec,
            mxasio::append(std::move(h),
...
#endif // MXASIO_EXPERIMENTAL_IMPL_CORO_HPP
