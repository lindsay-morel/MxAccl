...
#ifndef MXASIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
#define MXASIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
...
struct parallel_group_completion_handler
{
  ...
  parallel_group_completion_handler(Handler&& h)
    : handler_(std::move(h)),
      executor_(
          mxasio::prefer(
            mxasio::get_associated_executor(handler_),
  ...
  void operator()()
  {
    this->invoke(mxasio::detail::make_index_sequence<sizeof...(Ops)>());
  }

  template <std::size_t... I>
  void invoke(mxasio::detail::index_sequence<I...>)
  {
  ...
  void invoke(std::tuple<Args...>&& args)
  {
    this->invoke(std::move(args),
        mxasio::detail::index_sequence_for<Args...>());
  }

  template <typename... Args, std::size_t... I>
  void invoke(std::tuple<Args...>&& args,
      mxasio::detail::index_sequence<I...>)
  {
...
struct parallel_group_state
{
  ...
  mxasio::cancellation_signal cancellation_signals_[sizeof...(Ops)];
...
{
  typedef mxasio::cancellation_slot cancellation_slot_type;
  ...
  void operator()(Args... args)
  {
    ...
    if (--state_->outstanding_ == 0)
      mxasio::dispatch(std::move(state_->handler_));
  }
  ...
  parallel_group_op_handler<I, Condition, Handler, Ops...>
{
  ...
  struct cancel_proxy
  {
    ...
    void operator()(cancellation_type_t type)
    {
      ...
      {
        mxasio::cancellation_signal* sig = &signal_;
        mxasio::dispatch(executor_,
            [state, sig, type]{ sig->emit(type); });
      }
    }

    std::weak_ptr<parallel_group_state<Condition, Handler, Ops...>> state_;
    mxasio::cancellation_signal signal_;
    executor_type executor_;
  };
...
struct parallel_group_op_launcher
{
      ...
      Condition, Handler, Ops...>>& state)
  {
    ...
    ex_type ex = mxasio::get_associated_executor(op);
    std::move(op)(
    ...
    std::tuple<Ops...>& ops, mxasio::detail::index_sequence<I...>)
{
  ...
  std::shared_ptr<state_type> state = std::allocate_shared<state_type>(
      mxasio::detail::recycling_allocator<state_type,
        mxasio::detail::thread_info_base::parallel_group_tag>(),
...
struct ranged_parallel_group_completion_handler
{
  ...
  ranged_parallel_group_completion_handler(Handler&& h,
    ...
    : handler_(std::move(h)),
      ...
      completion_order_(size, 0,
          MXASIO_REBIND_ALLOC(Allocator, std::size_t)(allocator)),
      args_(MXASIO_REBIND_ALLOC(Allocator, op_result_type)(allocator))
  {
  ...
  {
    this->invoke(
        mxasio::detail::make_index_sequence<
  ...
  void invoke(mxasio::detail::index_sequence<I...>)
  {
    ...
    vectors_type vectors{
        typename std::tuple_element<I, vectors_type>::type(
          MXASIO_REBIND_ALLOC(Allocator, int)(allocator_))...};
  ...
  std::vector<std::size_t,
    MXASIO_REBIND_ALLOC(Allocator, std::size_t)> completion_order_;
  std::deque<op_result_type,
    MXASIO_REBIND_ALLOC(Allocator, op_result_type)> args_;
};
...
struct ranged_parallel_group_state
{
  ranged_parallel_group_state(Condition&& c, Handler&& h,
    ...
    : cancellations_requested_(size),
      ...
      cancellation_signals_(
          MXASIO_REBIND_ALLOC(Allocator,
            mxasio::cancellation_signal)(allocator)),
      cancellation_condition_(std::move(c)),
  ...
  // The cancellation signals for each operation in the group.
  std::deque<mxasio::cancellation_signal,
    MXASIO_REBIND_ALLOC(Allocator, mxasio::cancellation_signal)>
      cancellation_signals_;
...
#endif // MXASIO_IMPL_EXPERIMENTAL_PARALLEL_GROUP_HPP
