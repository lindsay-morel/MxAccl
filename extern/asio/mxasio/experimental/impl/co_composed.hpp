...
#ifndef MXASIO_IMPL_EXPERIMENTAL_CO_COMPOSED_HPP
#define MXASIO_IMPL_EXPERIMENTAL_CO_COMPOSED_HPP
...
#include "mxasio/detail/composed_work.hpp"
...
using mxasio::detail::composed_io_executors;
using mxasio::detail::composed_work;
using mxasio::detail::composed_work_guard;
using mxasio::detail::get_composed_io_executor;
using mxasio::detail::make_composed_io_executors;
using mxasio::detail::recycling_allocator;
using mxasio::detail::throw_error;
...
struct co_composed_state_default_cancellation_on_suspend_impl<Executors,
    Handler, Return, R(mxasio::error_code, Args...), Signatures...>
{
  using promise_type = co_composed_promise<Executors, Handler, Return>;
  using return_type = std::tuple<mxasio::error_code, Args...>;
  ...
  static constexpr void (*fn())(void*)
  {
    if constexpr ((is_constructible<Args>::value && ...))
    {
      return [](void* p)
      {
        ...
        std::move(handler)(
            mxasio::error_code(mxasio::error::operation_aborted),
            Args{}...);
      };
    }
    else
      ...
      return [](void* p)
      {
        ...
        std::move(handler)(
            std::make_exception_ptr(
              mxasio::system_error(
                mxasio::error::operation_aborted, "co_await")),
            Args{}...);
      };
    }
    else
...
class co_composed_handler<Executors, Handler,
    Return, R(mxasio::error_code, Args...)>
  : public co_composed_handler_base<Executors, Handler, Return>
{
public:
  using co_composed_handler_base<Executors,
  ...
  using result_type = std::tuple<mxasio::error_code, args_type>;

  template <typename... T>
  void operator()(const mxasio::error_code& ec, T&&... args)
  {
...
#if defined(MXASIO_ENABLE_HANDLER_TRACKING)
# if defined(MXASIO_HAS_SOURCE_LOCATION)
      , mxasio::detail::source_location location
        = mxasio::detail::source_location::current()
# endif // defined(MXASIO_HAS_SOURCE_LOCATION)
#endif // defined(MXASIO_ENABLE_HANDLER_TRACKING)
    )
  {
...
#if defined(MXASIO_ENABLE_HANDLER_TRACKING)
# if defined(MXASIO_HAS_SOURCE_LOCATION)
          , const mxasio::detail::source_location& location
# endif // defined(MXASIO_HAS_SOURCE_LOCATION)
#endif // defined(MXASIO_ENABLE_HANDLER_TRACKING)
        )
...
#if defined(MXASIO_ENABLE_HANDLER_TRACKING)
# if defined(MXASIO_HAS_SOURCE_LOCATION)
      mxasio::detail::source_location location_;
# endif // defined(MXASIO_HAS_SOURCE_LOCATION)
#endif // defined(MXASIO_ENABLE_HANDLER_TRACKING)
    };
...
struct coroutine_traits<void, C&,
    mxasio::experimental::detail::co_composed_state<
      Executors, Handler, Return>,
    Args...>
{
  using promise_type =
    mxasio::experimental::detail::co_composed_promise<
      Executors, Handler, Return>;
};
...
#endif // MXASIO_IMPL_EXPERIMENTAL_CO_COMPOSED_HPP
