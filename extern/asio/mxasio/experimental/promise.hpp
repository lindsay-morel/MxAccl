...
#ifndef MXASIO_EXPERIMENTAL_PROMISE_HPP
#define MXASIO_EXPERIMENTAL_PROMISE_HPP
...
#include "mxasio/experimental/impl/promise.hpp"
 ...
 * @code
 * awaitable<void> read_write_some(mxasio::ip::tcp::socket & sock,
 *     mxasio::mutable_buffer read_buf, mxasio::const_buffer to_write)
 * {
 *   auto p = mxasio::async_read(read_buf, mxasio::use_awaitable);
 *   co_await mxasio::async_write_some(to_write, mxasio::deferred);
 *   co_await p;
 * }
 * @endcode
 */
...
#endif // defined(GENERATING_DOCUMENTATION)
{
  ...
  void cancel(cancellation_type level = cancellation_type::all)
  {
    if (impl_ && !impl_->done)
    {
      mxasio::dispatch(impl_->executor,
  ...
  template <MXASIO_COMPLETION_TOKEN_FOR(void(Ts...)) CompletionToken>
  inline MXASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken, void(Ts...))
...
private:
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(WaitHandler&& handler) const
    {
      ...
      if (self_->done)
      {
        auto exec = mxasio::get_associated_executor(
...
#endif // MXASIO_EXPERIMENTAL_PROMISE_HPP
