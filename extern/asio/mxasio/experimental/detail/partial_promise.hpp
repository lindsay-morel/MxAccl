...
#ifndef MXASIO_EXPERIMENTAL_DETAIL_PARTIAL_PROMISE_HPP
#define MXASIO_EXPERIMENTAL_DETAIL_PARTIAL_PROMISE_HPP

#include "mxasio/detail/config.hpp"
#include "mxasio/append.hpp"
#include "mxasio/awaitable.hpp"
#include "mxasio/experimental/coro_traits.hpp"
...
struct partial_promise : partial_promise_base<Allocator>
{
  ...
  {
    return mxasio::detail::suspend_always{};
  }
  ...
  auto final_suspend() noexcept
  {
    struct awaitable_t
    {
      ...
      auto await_suspend(mxasio::detail::coroutine_handle<>) noexcept
      {
...
struct coroutine_traits<
    mxasio::experimental::detail::partial_coro,
    Executor, Completion, Args...>
{
  using promise_type =
    mxasio::experimental::detail::partial_promise<
      mxasio::associated_allocator_t<Completion>>;
};
...
{
  post(exec, mxasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro post_coroutine(Context& ctx,
    CompletionToken token, Args&&... args) noexcept
{
  post(ctx, mxasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Executor exec,
    CompletionToken token, Args&&... args) noexcept
{
  dispatch(exec, mxasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
partial_coro dispatch_coroutine(Context& ctx,
    CompletionToken token, Args &&... args) noexcept
{
  dispatch(ctx, mxasio::append(std::move(token), std::move(args)...));
  co_return;
}
...
#endif // MXASIO_EXPERIMENTAL_DETAIL_PARTIAL_PROMISE_HPP
