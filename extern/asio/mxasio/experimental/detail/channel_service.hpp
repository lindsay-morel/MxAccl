...
#ifndef MXASIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
#define MXASIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
...
#include "mxasio/experimental/detail/channel_receive_op.hpp"
#include "mxasio/experimental/detail/channel_send_op.hpp"
#include "mxasio/experimental/detail/has_signature.hpp"
...
class channel_service
  : public mxasio::detail::execution_context_service_base<
...
public:
  ...
  struct base_implementation_type
  {
    ...
    mxasio::detail::op_queue<channel_operation> waiters_;
  ...
  // Constructor.
  channel_service(mxasio::execution_context& ctx);
      ...
      Handler& handler, const IoExecutor& io_ex)
  {
    ...
    MXASIO_HANDLER_CREATION((this->context(), *p.p,
...
struct channel_service<Mutex>::implementation_type<
    Traits, R(mxasio::error_code)>
  : channel_service::base_implementation_type
{
  // The traits type associated with the channel.
  typedef typename Traits::template rebind<R(mxasio::error_code)>::other
    traits_type;
  ...
  typedef conditional_t<
      ...
      conditional_t<
        has_signature<
          ...
          R(mxasio::error_code)
        >::value,
        channel_payload<R(mxasio::error_code)>,
        channel_payload<
          R(mxasio::error_code),
          typename traits_type::receive_closed_signature
        >
  ...
  void buffer_push(payload_type payload)
  {
    ...
    {
      mxasio::error_code value{last.value_};
...
private:
  ...
  {
    mxasio::error_code value_;
    std::size_t count_;
  };

  struct value_handler
  {
    mxasio::error_code& target_;

    template <typename... Args>
    void operator()(const mxasio::error_code& value, Args&&...)
    {
      target_ = value;
    }
  };
...
#include "mxasio/experimental/detail/impl/channel_service.hpp"

#endif // MXASIO_EXPERIMENTAL_DETAIL_CHANNEL_SERVICE_HPP
