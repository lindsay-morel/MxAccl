...
#ifndef MXASIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
#define MXASIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
...
auto coro_interpret_result(
    std::tuple<mxasio::error_code, Args...>&& args)
{
  if (std::get<0>(args))
    mxasio::detail::throw_exception(
        mxasio::system_error(std::get<0>(args)));
...
inline auto coro_interpret_result(
    std::tuple<mxasio::error_code>&& args)
{
  if (std::get<0>(args))
    mxasio::detail::throw_exception(
        mxasio::system_error(std::get<0>(args)));
...
template <typename Arg>
auto coro_interpret_result(std::tuple<mxasio::error_code, Arg>&& args)
{
  if (std::get<0>(args))
    mxasio::detail::throw_exception(
        mxasio::system_error(std::get<0>(args)));
...
#endif // MXASIO_EXPERIMENTAL_DETAIL_CORO_COMPLETION_HANDLER_HPP
