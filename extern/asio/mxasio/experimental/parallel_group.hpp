...
#ifndef MXASIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
#define MXASIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
...
#include "mxasio/detail/utility.hpp"
#include "mxasio/experimental/cancellation_condition.hpp"
...
{
  typedef mxasio::detail::array<std::size_t, N> order_type;
...
void parallel_group_launch(Condition cancellation_condition, Handler handler,
    std::tuple<Ops...>& ops, mxasio::detail::index_sequence<I...>);
...
struct ranged_parallel_group_signature<R(Args...), Allocator>
{
  typedef std::vector<std::size_t,
    MXASIO_REBIND_ALLOC(Allocator, std::size_t)> order_type;
  typedef R raw_type(
      std::vector<Args, MXASIO_REBIND_ALLOC(Allocator, Args)>...);
...
/**
 * See the documentation for mxasio::experimental::make_parallel_group for
 * a usage example.
 */
  ...
  struct initiate_async_wait
  {
    ...
    void operator()(Handler&& h, Condition&& c, std::tuple<Ops...>&& ops) const
    {
      detail::parallel_group_launch(
          ...
          ops, mxasio::detail::index_sequence_for<Ops...>());
    }
  };
...
public:
   ...
   * operations, it must return a mxasio::cancellation_type value other
   * than <tt>mxasio::cancellation_type::none</tt>.
   *
   ...
   *
   * @li mxasio::experimental::wait_for_all
   * @li mxasio::experimental::wait_for_one
   * @li mxasio::experimental::wait_for_one_error
   * @li mxasio::experimental::wait_for_one_success
   */
  template <typename CancellationCondition,
      MXASIO_COMPLETION_TOKEN_FOR(signature) CompletionToken>
  auto async_wait(CancellationCondition cancellation_condition,
      CompletionToken&& token)
    -> decltype(
      mxasio::async_initiate<CompletionToken, signature>(
        declval<initiate_async_wait>(), token,
        std::move(cancellation_condition), std::move(ops_)))
  {
    return mxasio::async_initiate<CompletionToken, signature>(
...
/**
 * For example:
 * @code mxasio::experimental::make_parallel_group(
 *    [&](auto token)
 *    {
 *      return in.async_read_some(mxasio::buffer(data), token);
 *    },
 *    [&](auto token)
 *    {
 ...
 *    }
 *  ).async_wait(
 *    mxasio::experimental::wait_for_all(),
 *    [](
 ...
 */
template <typename... Ops>
MXASIO_NODISCARD inline parallel_group<Ops...>
 ...
 *   );
 *
 * mxasio::experimental::make_parallel_group(ops).async_wait(
 *     mxasio::experimental::wait_for_all(),
 *     [](
...
MXASIO_NODISCARD inline ranged_parallel_group<decay_t<Range>>
 ...
 *   );
 *
 * mxasio::experimental::make_parallel_group(
 *     std::allocator_arg_t,
 *     my_allocator,
 *     ops
 *   ).async_wait(
...
MXASIO_NODISCARD inline ranged_parallel_group<decay_t<Range>, Allocator>
...
#include "mxasio/experimental/impl/parallel_group.hpp"

#endif // MXASIO_EXPERIMENTAL_PARALLEL_GROUP_HPP
