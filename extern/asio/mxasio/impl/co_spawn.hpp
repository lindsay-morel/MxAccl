...
#ifndef MXASIO_IMPL_CO_SPAWN_HPP
#define MXASIO_IMPL_CO_SPAWN_HPP
...
struct co_spawn_state
{
  ...
  co_spawn_state(H&& h, const Executor& ex, F&& f)
    : handler(std::forward<H>(h)),
      spawn_work(ex),
      handler_work(mxasio::get_associated_executor(handler, ex)),
...
struct co_spawn_dispatch
{
  ...
  auto operator()(CompletionToken&& token) const
    -> decltype(mxasio::dispatch(std::forward<CompletionToken>(token)))
  {
    return mxasio::dispatch(std::forward<CompletionToken>(token));
  }
};
...
struct co_spawn_post
{
  template <typename CompletionToken>
  auto operator()(CompletionToken&& token) const
    -> decltype(mxasio::post(std::forward<CompletionToken>(token)))
  {
    return mxasio::post(std::forward<CompletionToken>(token));
  }
};
...
public:
  ...
  void operator()(cancellation_type_t type)
  {
    ...
    mxasio::dispatch(ex_, [sig, type]{ sig->emit(type); });
  }
...
public:
  ...
  void operator()(Handler&& handler, F&& f) const
  {
    ...
    auto slot = mxasio::get_associated_cancellation_slot(handler);
...
inline MXASIO_INITFN_AUTO_RESULT_TYPE(CompletionToken,
...
#endif // MXASIO_IMPL_CO_SPAWN_HPP
