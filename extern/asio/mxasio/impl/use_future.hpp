...
#ifndef MXASIO_IMPL_USE_FUTURE_HPP
#define MXASIO_IMPL_USE_FUTURE_HPP
...
public:
  ...
  {
    return mxasio::query(system_executor(), execution::context);
  }
  ...
  {
    mxasio::require(system_executor(), Blocking()).execute(
...
protected:
  ...
  {
    MXASIO_REBIND_ALLOC(Allocator, char) b(a);
...
{
public:
  void operator()(const mxasio::error_code& ec)
  {
    if (ec)
    {
      this->p_->set_exception(
          std::make_exception_ptr(
            mxasio::system_error(ec)));
    }
    else
    {
      this->p_->set_value();
...
template <>
class promise_handler_selector<void(mxasio::error_code)>
...
template <typename Arg>
class promise_handler_selector<void(mxasio::error_code, Arg)>
...
struct equality_comparable<
    mxasio::detail::promise_executor<T, Blocking>>
{
...
struct execute_member<
    mxasio::detail::promise_executor<T, Blocking>, Function>
{
  static constexpr bool is_valid = true;
  ...
  >
{
  static constexpr bool is_valid = true;
  static constexpr bool is_noexcept = true;
  typedef mxasio::detail::promise_executor<T,
...
#endif // MXASIO_IMPL_USE_FUTURE_HPP
