...
#ifndef MXASIO_IMPL_SPAWN_HPP
#define MXASIO_IMPL_SPAWN_HPP
  ...
  void suspend_with(void (*fn)(void*), void* arg)
  {
    if (throw_if_cancelled_)
      if (!!cancellation_state_.cancelled())
        throw_error(mxasio::error::operation_aborted, "yield");
...
#elif defined(MXASIO_HAS_BOOST_COROUTINE)
...
template <typename Executor, typename R>
class spawn_handler<Executor, R(mxasio::error_code)>
  : public spawn_handler_base<Executor>
{
public:
  typedef void return_type;
...
template <typename Executor, typename R, typename T>
class spawn_handler<Executor, R(mxasio::error_code, T)>
  : public spawn_handler_base<Executor>
{
public:
  typedef T return_type;
...
template <typename Executor, typename R, typename... Ts>
class spawn_handler<Executor, R(mxasio::error_code, Ts...)>
  : public spawn_handler_base<Executor>
{
public:
  typedef std::tuple<Ts...> return_type;
...
#else // defined(MXASIO_HAS_VARIADIC_LAMBDA_CAPTURES)
...
template <typename Executor, typename F,
    MXASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<
    ...
    const boost::coroutines::attributes& attributes)
{
  ...
  mxasio::spawn(ex, static_cast<Function&&>(function), attributes);
}
...
inline void spawn(const Executor& ex, Function&& function,
    ...
    >)
{
  mxasio::spawn(mxasio::strand<Executor>(ex),
...
inline void spawn(const strand<Executor>& ex, Function&& function,
    const boost::coroutines::attributes& attributes)
{
  mxasio::spawn(mxasio::bind_executor(
...
template <typename Function>
inline void spawn(const mxasio::io_context::strand& s, Function&& function,
    const boost::coroutines::attributes& attributes)
{
  mxasio::spawn(mxasio::bind_executor(
