...
#ifndef MXASIO_IMPL_AWAITABLE_HPP
#define MXASIO_IMPL_AWAITABLE_HPP
...
#include "mxasio/detail/source_location.hpp"
...
{
public:
#if !defined(MXASIO_DISABLE_AWAITABLE_FRAME_RECYCLING)
  void* operator new(std::size_t size)
  {
    return mxasio::detail::thread_info_base::allocate(
        mxasio::detail::thread_info_base::awaitable_frame_tag(),
...
#endif // !defined(MXASIO_DISABLE_AWAITABLE_FRAME_RECYCLING)
  ...
  void set_error(const mxasio::error_code& ec)
  {
    this->set_except(std::make_exception_ptr(mxasio::system_error(ec)));
  }
  ...
  auto await_transform(awaitable<T, Executor> a) const
  {
    if (attached_thread_->entry_point()->throw_if_cancelled_)
      if (!!attached_thread_->get_cancellation_state().cancelled())
        throw_error(mxasio::error::operation_aborted, "co_await");
    return a;
  }
...
private:
  ...
  mxasio::cancellation_slot parent_cancellation_slot_;
  mxasio::cancellation_state cancellation_state_;
  bool has_executor_;
...
template <typename R, typename Executor>
class awaitable_async_op_handler<R(mxasio::error_code), Executor>
  : public awaitable_thread<Executor>
{
public:
  typedef mxasio::error_code* result_type;
...
template <typename R, typename T, typename Executor>
class awaitable_async_op_handler<R(mxasio::error_code, T), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  void operator()(mxasio::error_code ec, T value)
  {
    result_.ec_ = &ec;
...
template <typename R, typename... Ts, typename Executor>
class awaitable_async_op_handler<R(mxasio::error_code, Ts...), Executor>
  : public awaitable_thread<Executor>
{
public:
  struct result_type
  ...
  template <typename... Args>
  void operator()(mxasio::error_code ec, Args&&... args)
  {
    result_.ec_ = &ec;
...
#if defined(MXASIO_ENABLE_HANDLER_TRACKING)
# if defined(MXASIO_HAS_SOURCE_LOCATION)
          MXASIO_HANDLER_LOCATION((self->location_.file_name(),
...
struct coroutine_traits<mxasio::awaitable<T, Executor>, Args...>
{
  typedef mxasio::detail::awaitable_frame<T, Executor> promise_type;
};

} // namespace std

# else // defined(MXASIO_HAS_STD_COROUTINE)
...
#endif // MXASIO_IMPL_AWAITABLE_HPP
