...
#ifndef MXASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
#define MXASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
...
template <typename Stream>
std::size_t buffered_write_stream<Stream>::flush(mxasio::error_code& ec)
{
  std::size_t bytes_written = write(next_layer_,
      buffer(storage_.data(), storage_.size()),
      transfer_all(), ec);
...
namespace detail
{
  ...
  public:
    ...
    void operator()(const mxasio::error_code& ec, std::size_t)
    {
      if (ec)
      {
      ...
      else
      {
        ...
        const std::size_t bytes_copied = mxasio::buffer_copy(
  ...
  public:
        ...
        const ConstBufferSequence& buffers) const
    {
      ...
      {
        next_layer_.async_write_some(MXASIO_CONST_BUFFER(0, 0),
...
#endif // MXASIO_IMPL_BUFFERED_WRITE_STREAM_HPP
