...
#ifndef MXASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
#define MXASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
...
#include "mxasio/detail/win_iocp_socket_service_base.hpp"
...
mxasio::error_code win_iocp_socket_service_base::close(
    win_iocp_socket_service_base::base_implementation_type& impl,
    mxasio::error_code& ec)
{
  if (is_open(impl))
  {
    MXASIO_HANDLER_OPERATION((iocp_service_.context(),
...
mxasio::error_code win_iocp_socket_service_base::cancel(
    win_iocp_socket_service_base::base_implementation_type& impl,
    mxasio::error_code& ec)
{
  if (!is_open(impl))
        ...
        ::GetModuleHandleA("KERNEL32"), "CancelIoEx"))
  {
    ...
    if (!cancel_io_ex(sock_as_handle, 0))
    {
      ...
      {
        ec = mxasio::error_code(last_error,
...
#else // defined(MXASIO_ENABLE_CANCELIO)
  else
  {
...
mxasio::error_code win_iocp_socket_service_base::do_open(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int family, int type, int protocol, mxasio::error_code& ec)
{
  if (is_open(impl))
  {
...
mxasio::error_code win_iocp_socket_service_base::do_assign(
    win_iocp_socket_service_base::base_implementation_type& impl,
    int type, socket_type native_socket, mxasio::error_code& ec)
{
  if (is_open(impl))
  {
    ec = mxasio::error::already_open;
    ...
    socket_base::message_flags flags, bool noop, operation* op)
{
  ...
  if (noop)
    ...
    iocp_service_.on_completion(op, mxasio::error::bad_descriptor);
  else
  {
    ...
    int protocol, void* output_buffer, DWORD address_length, operation* op)
{
  ...
  if (!is_open(impl))
    ...
    iocp_service_.on_completion(op, mxasio::error::already_open);
  else
  {
    mxasio::error_code ec;
    ...
    long* cancel_requested, operation* op)
{
  ...
  if (cancel_requested)
    if (::InterlockedExchangeAdd(cancel_requested, 0) == 1)
      iocp_service_.on_completion(op, mxasio::error::operation_aborted);
    ...
    win_iocp_socket_connect_op_base* op, operation* iocp_op)
{
  ...
  if (family == MXASIO_OS_DEF(AF_INET)
      || family == MXASIO_OS_DEF(AF_INET6))
  {
    if (connect_ex_fn connect_ex = get_connect_ex(impl, type))
    {
      ...
      socket_ops::bind(impl.socket_, &a.base,
          family == MXASIO_OS_DEF(AF_INET)
          ? sizeof(a.v4) : sizeof(a.v6), op->ec_);
      if (op->ec_ && op->ec_ != mxasio::error::invalid_argument)
      {
        iocp_service_.post_immediate_completion(op, false);
        return -1;
      }
        ...
        impl.socket_, impl.state_, true, op->ec_))
  {
    ...
    {
      if (op->ec_ == mxasio::error::in_progress
          || op->ec_ == mxasio::error::would_block)
      {
...
{
#if defined(MXASIO_DISABLE_CONNECTEX)
  (void)impl;
  (void)type;
  return 0;
#else // defined(MXASIO_DISABLE_CONNECTEX)
  if (type != MXASIO_OS_DEF(SOCK_STREAM)
      && type != MXASIO_OS_DEF(SOCK_SEQPACKET))
    return 0;
...
#endif // defined(MXASIO_DISABLE_CONNECTEX)
}
...
#endif // MXASIO_DETAIL_IMPL_WIN_IOCP_SOCKET_SERVICE_BASE_IPP
