...
#ifndef MXASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
#define MXASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
...
dev_poll_reactor::dev_poll_reactor(mxasio::execution_context& ctx)
  : mxasio::detail::execution_context_service_base<dev_poll_reactor>(ctx),
...
void dev_poll_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  if (events_size > 0)
  {
    ...
    {
      mxasio::error_code ec = mxasio::error_code(
...
int dev_poll_reactor::do_dev_poll_create()
{
  ...
  if (fd == -1)
  {
    mxasio::error_code ec(errno,
        mxasio::error::get_system_category());
    mxasio::detail::throw_error(ec, "/dev/poll");
  }
  return fd;
}
...
#endif // MXASIO_DETAIL_IMPL_DEV_POLL_REACTOR_IPP
