...
#ifndef MXASIO_DETAIL_IMPL_DESCRIPTOR_OPS_IPP
#define MXASIO_DETAIL_IMPL_DESCRIPTOR_OPS_IPP
...
int open(const char* path, int flags, mxasio::error_code& ec)
{
...
int open(const char* path, int flags,
    unsigned mode, mxasio::error_code& ec)
{
...
int close(int d, state_type& state, mxasio::error_code& ec)
{
  int result = 0;
  if (d != -1)
  {
...
bool set_user_non_blocking(int d, state_type& state,
    bool value, mxasio::error_code& ec)
{
  if (d == -1)
  {
...
std::size_t sync_read_at(int d, state_type state, uint64_t offset,
    buf* bufs, std::size_t count, bool all_empty, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
int fcntl(int d, int cmd, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
int fcntl(int d, int cmd, long arg, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
int poll_read(int d, state_type state, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
int poll_write(int d, state_type state, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
int poll_error(int d, state_type state, mxasio::error_code& ec)
{
  if (d == -1)
  {
    ec = mxasio::error::bad_descriptor;
...
#endif // MXASIO_DETAIL_IMPL_DESCRIPTOR_OPS_IPP
