...
#ifndef MXASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
#define MXASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
...
#include <cstring>
#include <stdexcept>
#include "mxasio/detail/signal_blocker.hpp"
...
signal_state* get_signal_state()
{
  static signal_state state = {
    MXASIO_STATIC_MUTEX_INIT, -1, -1, false, 0,
...
#if defined(MXASIO_HAS_SIGNAL) && !defined(MXASIO_HAS_SIGACTION)
  ::signal(signal_number, asio_signal_handler);
#endif // defined(MXASIO_HAS_SIGNAL) && !defined(MXASIO_HAS_SIGACTION)
}
...
# if defined(MXASIO_HAS_IO_URING_AS_DEFAULT)
  pipe_read_op()
    : io_uring_operation(mxasio::error_code(), &pipe_read_op::do_prepare,
...
signal_set_service::signal_set_service(execution_context& context)
  : execution_context_service_base<signal_set_service>(context),
    scheduler_(mxasio::use_service<scheduler_impl>(context)),
#if !defined(MXASIO_WINDOWS) \
  && !defined(MXASIO_WINDOWS_RUNTIME) \
  && !defined(__CYGWIN__)
# if defined(MXASIO_HAS_IO_URING_AS_DEFAULT)
    io_uring_service_(mxasio::use_service<io_uring_service>(context)),
# else // defined(MXASIO_HAS_IO_URING_AS_DEFAULT)
...
# endif // defined(MXASIO_HAS_IO_URING_AS_DEFAULT)
  ...
  case execution_context::fork_child:
    if (state->fork_prepared_)
    {
      mxasio::detail::signal_blocker blocker;
      close_descriptors();
      open_descriptors();
      int read_descriptor = state->read_descriptor_;
      state->fork_prepared_ = false;
...
mxasio::error_code signal_set_service::add(
    signal_set_service::implementation_type& impl, int signal_number,
    signal_set_base::flags_t f, mxasio::error_code& ec)
{
...
# else // defined(MXASIO_HAS_SIGACTION)
...
mxasio::error_code signal_set_service::remove(
    signal_set_service::implementation_type& impl,
...
mxasio::error_code signal_set_service::clear(
    signal_set_service::implementation_type& impl,
...
mxasio::error_code signal_set_service::cancel(
    signal_set_service::implementation_type& impl,
    mxasio::error_code& ec)
{
  MXASIO_HANDLER_OPERATION((scheduler_.context(),
...
void signal_set_service::add_service(signal_set_service* service)
{
  ...
  if (state->service_list_ != 0)
  {
    if (!MXASIO_CONCURRENCY_HINT_IS_LOCKING(SCHEDULER,
          service->scheduler_.concurrency_hint())
        || !MXASIO_CONCURRENCY_HINT_IS_LOCKING(SCHEDULER,
...
void signal_set_service::open_descriptors()
{
  ...
  else
  {
    mxasio::error_code ec(errno,
        mxasio::error::get_system_category());
    mxasio::detail::throw_error(ec, "signal_set_service pipe");
  }
#endif // !defined(MXASIO_WINDOWS)
       //   && !defined(MXASIO_WINDOWS_RUNTIME)
       //   && !defined(__CYGWIN__)
...
#endif // MXASIO_DETAIL_IMPL_SIGNAL_SET_SERVICE_IPP
