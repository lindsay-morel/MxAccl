...
#ifndef MXASIO_DETAIL_SOCKET_OPS_IPP
#define MXASIO_DETAIL_SOCKET_OPS_IPP
    ...
    mxasio::error_code& ec, bool is_error_condition)
{
  ...
  {
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
    ec = mxasio::error_code(WSAGetLastError(),
    ...
    void* addr, std::size_t* addrlen, mxasio::error_code& ec)
{
  ...
  for (;;)
  {
    ...
    else if (ec == mxasio::error::connection_aborted)
    {
    ...
    socket_type new_socket, mxasio::error_code& ec)
{
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = mxasio::error::connection_aborted;
  ...
  if (!ec)
  {
    ...
    if (addr && addrlen)
    {
      ...
      {
        ec = mxasio::error::invalid_argument;
      }
      else
      {
...
int shutdown(socket_type s, int what, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
    ...
    std::size_t addrlen, mxasio::error_code& ec)
{
  ...
  if (result != 0 && ec == mxasio::error::try_again)
  {
    if (static_cast<const socket_addr_type*>(addr)->sa_family == AF_UNIX)
      ec = mxasio::error::in_progress;
    else
      ec = mxasio::error::no_buffer_space;
  }
    ...
    std::size_t addrlen, mxasio::error_code& ec)
{
  ...
  if (ec != mxasio::error::in_progress
      && ec != mxasio::error::would_block)
  {
  ...
  ec = mxasio::error_code(connect_error,
      mxasio::error::get_system_category());
}

#if defined(MXASIO_HAS_IOCP)

void complete_iocp_connect(socket_type s, mxasio::error_code& ec)
{
  // Map non-portable errors to their portable counterparts.
  switch (ec.value())
  {
  case ERROR_CONNECTION_REFUSED:
    ec = mxasio::error::connection_refused;
    break;
  case ERROR_NETWORK_UNREACHABLE:
    ec = mxasio::error::network_unreachable;
    break;
  case ERROR_HOST_UNREACHABLE:
    ec = mxasio::error::host_unreachable;
    break;
  case ERROR_SEM_TIMEOUT:
    ec = mxasio::error::timed_out;
    break;
  default:
    break;
  }
...
bool non_blocking_connect(socket_type s, mxasio::error_code& ec)
{
...
bool sockatmark(socket_type s, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
size_t available(socket_type s, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
int listen(socket_type s, int backlog, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
signed_size_type recv(socket_type s, buf* bufs, size_t count,
    int flags, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
    ec = mxasio::error::connection_reset;
...
size_t sync_recv1(socket_type s, state_type state, void* data,
    size_t size, int flags, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
    ...
    mxasio::error_code& ec, size_t bytes_transferred)
{
  ...
  if (ec.value() == ERROR_NETNAME_DELETED)
  {
    if (cancel_token.expired())
      ec = mxasio::error::operation_aborted;
    else
      ec = mxasio::error::connection_reset;
  }
  else if (ec.value() == ERROR_PORT_UNREACHABLE)
...
signed_size_type recvfrom(socket_type s, buf* bufs, size_t count,
    int flags, void* addr, std::size_t* addrlen, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  // Receive some data.
  DWORD recv_buf_count = static_cast<DWORD>(count);
...
signed_size_type recvmsg(socket_type s, buf* bufs, size_t count,
    int in_flags, int& out_flags, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  out_flags = 0;
...
int setsockopt(socket_type s, state_type& state, int level, int optname,
    const void* optval, std::size_t optlen, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
int getsockopt(socket_type s, state_type state, int level, int optname,
    void* optval, size_t* optlen, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
int getpeername(socket_type s, void* addr, std::size_t* addrlen,
    bool cached, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
...
#if defined(MXASIO_WINDOWS) && !defined(MXASIO_WINDOWS_APP) \
  || defined(__CYGWIN__)
  if (cached)
  {
...
#else // defined(MXASIO_WINDOWS) && !defined(MXASIO_WINDOWS_APP)
      // || defined(__CYGWIN__)
  (void)cached;
#endif // defined(MXASIO_WINDOWS) && !defined(MXASIO_WINDOWS_APP)
       // || defined(__CYGWIN__)
...
int poll_connect(socket_type s, int msec, mxasio::error_code& ec)
{
  if (s == invalid_socket)
  {
    ec = mxasio::error::bad_descriptor;
    ...
    unsigned long scope_id, mxasio::error_code& ec)
{
  ...
  if (af == MXASIO_OS_DEF(AF_INET))
  {
    sprintf_s(dest, length, "%u.%u.%u.%u",
        bytes[0], bytes[1], bytes[2], bytes[3]);
    return dest;
  }
  else if (af == MXASIO_OS_DEF(AF_INET6))
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length && b < 16)
    {
  ...
  {
    ec = mxasio::error::address_family_not_supported;
    return 0;
  }
#elif defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  using namespace std; // For memcpy.

  if (af != MXASIO_OS_DEF(AF_INET) && af != MXASIO_OS_DEF(AF_INET6))
  {
    ec = mxasio::error::address_family_not_supported;
    return 0;
  }
  ...
  if (af == MXASIO_OS_DEF(AF_INET))
  {
    ...
    address.v4.sin_family = MXASIO_OS_DEF(AF_INET);
  ...
  else // AF_INET6
  {
    ...
    address.v6.sin6_family = MXASIO_OS_DEF(AF_INET6);
  ...
  if (result == 0 && !ec)
    ec = mxasio::error::invalid_argument;
  if (result != 0 && af == MXASIO_OS_DEF(AF_INET6) && scope_id != 0)
  {
...
#else // defined(MXASIO_HAS_SNPRINTF)
    ...
    unsigned long* scope_id, mxasio::error_code& ec)
{
  ...
  const bool is_v6 = (af == MXASIO_OS_DEF(AF_INET6));
...
int gethostname(char* name, int namelen, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS_RUNTIME)
  try
  {
  ...
  {
    ec = mxasio::error_code(e->HResult,
        mxasio::system_category());
    return -1;
  }
#else // defined(MXASIO_WINDOWS_RUNTIME)
...
inline mxasio::error_code translate_netdb_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case HOST_NOT_FOUND:
    return mxasio::error::host_not_found;
  case TRY_AGAIN:
    return mxasio::error::host_not_found_try_again;
  case NO_RECOVERY:
    return mxasio::error::no_recovery;
  case NO_DATA:
    return mxasio::error::no_data;
  default:
    MXASIO_ASSERT(false);
    return mxasio::error::invalid_argument;
  }
}

inline hostent* gethostbyaddr(const char* addr, int length, int af,
    hostent* result, char* buffer, int buflength, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
...
inline hostent* gethostbyname(const char* name, int af, struct hostent* result,
    char* buffer, int buflength, int ai_flags, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
  (void)(buffer);
  (void)(buflength);
    ...
    const addrinfo_type* hints, gai_search (&search)[2])
{
  ...
  if (host == 0 || host[0] == '\0')
  {
    if (hints->ai_flags & AI_PASSIVE)
    {
      ...
      {
      case MXASIO_OS_DEF(AF_INET):
        search[search_count].host = "0.0.0.0";
        search[search_count].family = MXASIO_OS_DEF(AF_INET);
        ++search_count;
        break;
      case MXASIO_OS_DEF(AF_INET6):
        search[search_count].host = "0::0";
        search[search_count].family = MXASIO_OS_DEF(AF_INET6);
        ++search_count;
        break;
      case MXASIO_OS_DEF(AF_UNSPEC):
        search[search_count].host = "0::0";
        search[search_count].family = MXASIO_OS_DEF(AF_INET6);
        ++search_count;
        search[search_count].host = "0.0.0.0";
...
#else // defined(MXASIO_HAS_SECURE_RTL)
  *target = 0;
  if (max_size > 0)
  ...
  case MXASIO_OS_DEF(AF_INET):
    {
      ...
      if (sinptr == 0)
        return EAI_MEMORY;
      sinptr->sin_family = MXASIO_OS_DEF(AF_INET);
  ...
  case MXASIO_OS_DEF(AF_INET6):
    {
      ...
      if (sin6ptr == 0)
        return EAI_MEMORY;
      sin6ptr->sin6_family = MXASIO_OS_DEF(AF_INET6);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  hints.ai_family = MXASIO_OS_DEF(AF_UNSPEC);
  if (hintsp)
    hints = *hintsp;
  ...
  if (hints.ai_family != MXASIO_OS_DEF(AF_INET6))
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    mxasio::error_code ec;
    if (socket_ops::inet_pton(MXASIO_OS_DEF(AF_INET),
          sptr->host, &inaddr, 0, ec) == 1)
    {
      if (hints.ai_family != MXASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != MXASIO_OS_DEF(AF_INET))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == MXASIO_OS_DEF(AF_INET))
      {
        rc = gai_aistruct(&ainext, &hints, &inaddr, MXASIO_OS_DEF(AF_INET));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (socket_ops::inet_pton(MXASIO_OS_DEF(AF_INET6),
          sptr->host, &in6addr, 0, ec) == 1)
    {
      if (hints.ai_family != MXASIO_OS_DEF(AF_UNSPEC)
          && hints.ai_family != MXASIO_OS_DEF(AF_INET6))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == MXASIO_OS_DEF(AF_INET6))
      {
        rc = gai_aistruct(&ainext, &hints, &in6addr,
            MXASIO_OS_DEF(AF_INET6));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (hptr == 0)
    {
      ...
      freeaddrinfo_emulation(aihead);
      gai_free(canon);
      if (ec == mxasio::error::host_not_found)
        return EAI_NONAME;
      if (ec == mxasio::error::host_not_found_try_again)
        return EAI_AGAIN;
      if (ec == mxasio::error::no_recovery)
        return EAI_FAIL;
      if (ec == mxasio::error::no_data)
        return EAI_NONAME;
      return EAI_NONAME;
    }
...
inline mxasio::error_code getnameinfo_emulation(
    ...
    mxasio::error_code& ec)
{
  ...
  case MXASIO_OS_DEF(AF_INET):
    ...
    {
      return ec = mxasio::error::invalid_argument;
    }
  ...
  case MXASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    return ec = mxasio::error::address_family_not_supported;
  }
  ...
  if (host && hostlen > 0)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          return ec = mxasio::error::host_not_found;
        }
  ...
  if (serv && servlen > 0)
  {
    if (flags & NI_NUMERICSERV)
    {
      if (servlen < 6)
      {
        return ec = mxasio::error::no_buffer_space;
      }
#if defined(MXASIO_HAS_SNPRINTF)
      snprintf(serv, servlen, "%u", ntohs(port));
#elif defined(MXASIO_HAS_SECURE_RTL)
...
inline mxasio::error_code translate_addrinfo_error(int error)
{
  switch (error)
  {
  case 0:
  ...
  case EAI_FAIL:
    return mxasio::error::no_recovery;
  case EAI_FAMILY:
    return mxasio::error::address_family_not_supported;
  case EAI_MEMORY:
    return mxasio::error::no_memory;
  case EAI_NONAME:
  ...
  case EAI_NODATA:
    ...
    return mxasio::error::host_not_found;
  case EAI_SERVICE:
    return mxasio::error::service_not_found;
  case EAI_SOCKTYPE:
    return mxasio::error::socket_type_not_supported;
  default: // Possibly the non-portable EAI_SYSTEM.
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
    return mxasio::error_code(
        WSAGetLastError(), mxasio::error::get_system_category());
#else
    return mxasio::error_code(
        errno, mxasio::error::get_system_category());
#endif
  }
}

mxasio::error_code getaddrinfo(const char* host,
...
# else
  ...
  return ec = translate_addrinfo_error(error);
# endif
#elif !defined(MXASIO_HAS_GETADDRINFO)
  int error = getaddrinfo_emulation(host, service, &hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, &hints, result);
...
mxasio::error_code background_getaddrinfo(
...
mxasio::error_code getnameinfo(const void* addr,
    std::size_t addrlen, char* host, std::size_t hostlen,
    char* serv, std::size_t servlen, int flags, mxasio::error_code& ec)
{
#if defined(MXASIO_WINDOWS) || defined(__CYGWIN__)
# if defined(MXASIO_HAS_GETADDRINFO)
  // Building for Windows XP, Windows Server 2003, or later.
...
mxasio::error_code sync_getnameinfo(const void* addr,
...
mxasio::error_code background_getnameinfo(
    const weak_cancel_token_type& cancel_token,
    const void* addr, std::size_t addrlen,
...
#endif // MXASIO_DETAIL_SOCKET_OPS_IPP
