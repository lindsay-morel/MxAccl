...
#ifndef MXASIO_DETAIL_IMPL_SELECT_REACTOR_IPP
#define MXASIO_DETAIL_IMPL_SELECT_REACTOR_IPP
...
#include "mxasio/detail/config.hpp"
  ...
  || (!defined(MXASIO_HAS_DEV_POLL) \
      && !defined(MXASIO_HAS_EPOLL) \
      && !defined(MXASIO_HAS_KQUEUE) \
      && !defined(MXASIO_WINDOWS_RUNTIME))

#include "mxasio/detail/fd_set_adapter.hpp"
...
select_reactor::select_reactor(mxasio::execution_context& ctx)
...
{
  mxasio::detail::mutex::scoped_lock lock(mutex_);
  cancel_ops_unlocked(descriptor, mxasio::error::operation_aborted);
}
    ...
    int op_type, void* cancellation_key)
{
  ...
  bool need_interrupt = op_queue_[op_type].cancel_operations_by_key(
      descriptor, ops, cancellation_key, mxasio::error::operation_aborted);
    ...
    const mxasio::error_code& /*ec*/, std::size_t /*bytes_transferred*/)
{
  if (owner)
  {
    ...
    mxasio::detail::mutex::scoped_lock lock(reactor->mutex_);
    ...
    mxasio::detail::signal_blocker sb;
    reactor->thread_ =
      new mxasio::detail::thread(thread_function(reactor));
  }
}
#endif // defined(MXASIO_HAS_IOCP)
...
#endif // MXASIO_DETAIL_IMPL_SELECT_REACTOR_IPP
