...
#ifndef MXASIO_DETAIL_IMPL_SCHEDULER_IPP
#define MXASIO_DETAIL_IMPL_SCHEDULER_IPP
...
#include "mxasio/detail/scheduler_thread_info.hpp"
...
scheduler::scheduler(mxasio::execution_context& ctx,
    int concurrency_hint, bool own_thread, get_task_func_type get_task)
  : mxasio::detail::execution_context_service_base<scheduler>(ctx),
    one_thread_(concurrency_hint == 1
        || !MXASIO_CONCURRENCY_HINT_IS_LOCKING(
    ...
    thread_(0)
{
  ...
  {
    ++outstanding_work_;
    mxasio::detail::signal_blocker sb;
    thread_ = new mxasio::detail::thread(thread_function(this));
  }
}
...
std::size_t scheduler::run(mxasio::error_code& ec)
{
...
std::size_t scheduler::run_one(mxasio::error_code& ec)
{
  ec = mxasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::wait_one(long usec, mxasio::error_code& ec)
{
  ec = mxasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll(mxasio::error_code& ec)
{
  ec = mxasio::error_code();
  if (outstanding_work_ == 0)
  {
...
std::size_t scheduler::poll_one(mxasio::error_code& ec)
{
  ec = mxasio::error_code();
  if (outstanding_work_ == 0)
  {
...
void scheduler::compensating_work_started()
{
  ...
  MXASIO_ASSUME(this_thread != 0); // Only called from inside scheduler.
...
#else // defined(MXASIO_HAS_THREADS)
...
scheduler_task* scheduler::get_default_task(mxasio::execution_context& ctx)
{
#if defined(MXASIO_HAS_IO_URING_AS_DEFAULT)
...
#endif // MXASIO_DETAIL_IMPL_SCHEDULER_IPP
