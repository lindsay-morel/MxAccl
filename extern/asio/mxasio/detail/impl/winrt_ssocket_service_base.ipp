...
#ifndef MXASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
#define MXASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
...
#include <cstring>
#include "mxasio/detail/winrt_ssocket_service_base.hpp"
#include "mxasio/detail/winrt_async_op.hpp"
#include "mxasio/detail/winrt_utils.hpp"
    ...
    winrt_ssocket_service_base::base_implementation_type& other_impl)
{
  ...
  {
    // Insert implementation into linked list of all implementations.
    mxasio::detail::mutex::scoped_lock lock(other_service.mutex_);
...
mxasio::error_code winrt_ssocket_service_base::close(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    void* addr, std::size_t addr_len, mxasio::error_code& ec) const
{
  ...
  try
  {
    ...
    case MXASIO_OS_DEF(AF_INET):
      ...
      {
        socket_ops::inet_pton(MXASIO_OS_DEF(AF_INET), addr_string.c_str(),
    ...
    case MXASIO_OS_DEF(AF_INET6):
      ...
      {
        socket_ops::inet_pton(MXASIO_OS_DEF(AF_INET6), addr_string.c_str(),
...
mxasio::error_code winrt_ssocket_service_base::do_set_option(
    winrt_ssocket_service_base::base_implementation_type& impl,
    ...
    std::size_t optlen, mxasio::error_code& ec)
{
  ...
  try
  {
    if (level == MXASIO_OS_DEF(SOL_SOCKET)
        && optname == MXASIO_OS_DEF(SO_KEEPALIVE))
    {
    ...
    else if (level == MXASIO_OS_DEF(IPPROTO_TCP)
        && optname == MXASIO_OS_DEF(TCP_NODELAY))
    {
      if (optlen == sizeof(int))
      {
        int value = 0;
...
mxasio::error_code winrt_ssocket_service_base::do_connect(
    winrt_ssocket_service_base::base_implementation_type& impl,
    const void* addr, mxasio::error_code& ec)
{
  ...
  {
  case MXASIO_OS_DEF(AF_INET):
    socket_ops::inet_ntop(MXASIO_OS_DEF(AF_INET),
    ...
    break;
  case MXASIO_OS_DEF(AF_INET6):
    socket_ops::inet_ntop(MXASIO_OS_DEF(AF_INET6),
    ...
    const void* addr, winrt_async_op<void>* op, bool is_continuation)
{
  if (!is_open(impl))
  {
    op->ec_ = mxasio::error::bad_descriptor;
  ...
  case MXASIO_OS_DEF(AF_INET6):
    ...
    break;
  default:
    op->ec_ = mxasio::error::address_family_not_supported;
    break;
  }
  ...
  {
    op->ec_ = mxasio::error_code(
        e->HResult, mxasio::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, mxasio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<mxasio::const_buffer,
      mxasio::const_buffer> bufs(mxasio::buffer(data));
      ...
      winrt_async_op<unsigned int>* op, bool is_continuation)
{
  if (flags)
  {
    op->ec_ = mxasio::error::operation_not_supported;
    scheduler_.post_immediate_completion(op, is_continuation);
    return;
  }
  ...
  {
    op->ec_ = mxasio::error_code(e->HResult,
        mxasio::system_category());
    scheduler_.post_immediate_completion(op, is_continuation);
  }
}
    ...
    socket_base::message_flags flags, mxasio::error_code& ec)
{
  ...
  try
  {
    buffer_sequence_adapter<mxasio::mutable_buffer,
        mxasio::mutable_buffer> bufs(mxasio::buffer(data));
...
#endif // MXASIO_DETAIL_IMPL_WINRT_SSOCKET_SERVICE_BASE_IPP
