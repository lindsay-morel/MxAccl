...
#ifndef MXASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
#define MXASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
...
win_iocp_io_context::win_iocp_io_context(
    mxasio::execution_context& ctx, int concurrency_hint, bool own_thread)
    ...
    concurrency_hint_(concurrency_hint)
{
  ...
  if (!iocp_.handle)
  {
    ...
    mxasio::detail::throw_error(ec, "iocp");
  }

  if (own_thread)
  {
    ::InterlockedIncrement(&outstanding_work_);
    thread_.reset(new mxasio::detail::thread(thread_function(this)));
  }
}
...
mxasio::error_code win_iocp_io_context::register_handle(
...
size_t win_iocp_io_context::run(mxasio::error_code& ec)
{
...
size_t win_iocp_io_context::run_one(mxasio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::wait_one(long usec, mxasio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll(mxasio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
...
size_t win_iocp_io_context::poll_one(mxasio::error_code& ec)
{
  if (::InterlockedExchangeAdd(&outstanding_work_, 0) == 0)
  {
    stop();
  ...
  if (::InterlockedExchange(&stopped_, 1) == 0)
  {
    if (::InterlockedExchange(&stop_event_posted_, 1) == 0)
    {
      if (!::PostQueuedCompletionStatus(iocp_.handle, 0, 0, 0))
      {
        ...
        mxasio::error_code ec(last_error,
            mxasio::error::get_system_category());
        mxasio::detail::throw_error(ec, "pqcs");
      }
    }
  }
}
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const mxasio::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
    ...
    win_iocp_thread_info& this_thread, mxasio::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      mxasio::error_code result_ec(last_error,
          mxasio::error::get_system_category());
      ...
      {
        result_ec = mxasio::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<mxasio::error_category*>(op->Internal));
...
void win_iocp_io_context::do_add_timer_queue(timer_queue_base& queue)
{
  ...
  if (!waitable_timer_.handle)
  {
    ...
    if (waitable_timer_.handle == 0)
    {
      ...
      mxasio::error_code ec(last_error,
          mxasio::error::get_system_category());
      mxasio::detail::throw_error(ec, "timer");
    }
...
#endif // MXASIO_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
