...
#ifndef MXASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
#define MXASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
...
# define MXASIO_KQUEUE_EV_SET(ev, ident, filt, flags, fflags, data, udata) \
...
kqueue_reactor::kqueue_reactor(mxasio::execution_context& ctx)
  : execution_context_service_base<kqueue_reactor>(ctx),
    scheduler_(use_service<scheduler>(ctx)),
    mutex_(MXASIO_CONCURRENCY_HINT_IS_LOCKING(
    ...
    registered_descriptors_mutex_(mutex_.enabled())
{
  ...
  MXASIO_KQUEUE_EV_SET(&events[0], interrupter_.read_descriptor(),
      EVFILT_READ, EV_ADD, 0, 0, &interrupter_);
  if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
  {
    mxasio::error_code error(errno,
        mxasio::error::get_system_category());
    mxasio::detail::throw_error(error);
  }
}
...
{
  if (fork_ev == mxasio::execution_context::fork_child)
  {
    ...
    if (::kevent(kqueue_fd_, events, 1, 0, 0, 0) == -1)
    {
      mxasio::error_code ec(errno,
          mxasio::error::get_system_category());
      mxasio::detail::throw_error(ec, "kqueue interrupter registration");
    }
        ...
        state != 0; state = state->next_)
    {
      ...
      {
        MXASIO_KQUEUE_EV_SET(&events[0], state->descriptor_,
            EVFILT_READ, EV_ADD | EV_CLEAR, 0, 0, state);
        MXASIO_KQUEUE_EV_SET(&events[1], state->descriptor_,
            EVFILT_WRITE, EV_ADD | EV_CLEAR, 0, 0, state);
        if (::kevent(kqueue_fd_, events, state->num_kevents_, 0, 0, 0) == -1)
        {
          mxasio::error_code ec(errno,
              mxasio::error::get_system_category());
          mxasio::detail::throw_error(ec, "kqueue re-registration");
        }
      }
    }
  }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  MXASIO_HANDLER_REACTOR_REGISTRATION((
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, reactor_op* op)
{
  ...
  struct kevent events[1];
  MXASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
  ...
  if (descriptor_data->op_queue_[op_type].empty())
  {
          ...
          || descriptor_data->op_queue_[except_op].empty()))
    {
      ...
      if (descriptor_data->num_kevents_ < num_kevents[op_type])
      {
        ...
        MXASIO_KQUEUE_EV_SET(&events[0], descriptor, EVFILT_READ,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        MXASIO_KQUEUE_EV_SET(&events[1], descriptor, EVFILT_WRITE,
            EV_ADD | EV_CLEAR, 0, 0, descriptor_data);
        ...
        {
          op->ec_ = mxasio::error_code(errno,
              mxasio::error::get_system_category());
          on_immediate(op, is_continuation, immediate_arg);
          return;
        }
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data)
{
  ...
  for (int i = 0; i < max_ops; ++i)
  {
    ...
    {
      op->ec_ = mxasio::error::operation_aborted;
    ...
    kqueue_reactor::per_descriptor_data& descriptor_data, bool closing)
{
  ...
  if (!descriptor_data->shutdown_)
  {
    ...
    {
      struct kevent events[2];
      MXASIO_KQUEUE_EV_SET(&events[0], descriptor,
          EVFILT_READ, EV_DELETE, 0, 0, 0);
      MXASIO_KQUEUE_EV_SET(&events[1], descriptor,
    ...
    MXASIO_HANDLER_REACTOR_DEREGISTRATION((
          context(), static_cast<uintmax_t>(descriptor),
          reinterpret_cast<uintmax_t>(descriptor_data)));
...
void kqueue_reactor::run(long usec, op_queue<operation>& ops)
{
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    if (ptr != &interrupter_)
    {
      ...
      {
      case EVFILT_READ:
        event_mask |= MXASIO_HANDLER_REACTOR_READ_EVENT;
        break;
      case EVFILT_WRITE:
        event_mask |= MXASIO_HANDLER_REACTOR_WRITE_EVENT;
        break;
      }
      if ((events[i].flags & (EV_ERROR | EV_OOBAND)) != 0)
        event_mask |= MXASIO_HANDLER_REACTOR_ERROR_EVENT;
      MXASIO_HANDLER_REACTOR_EVENTS((context(),
  ...
  for (int i = 0; i < num_events; ++i)
  {
    ...
    else
    {
          ...
          && descriptor_data->op_queue_[write_op].empty())
      {
        ...
        MXASIO_KQUEUE_EV_SET(&delete_events[0],
...
int kqueue_reactor::do_kqueue_create()
{
  ...
  if (fd == -1)
  {
    mxasio::error_code ec(errno,
        mxasio::error::get_system_category());
    mxasio::detail::throw_error(ec, "kqueue");
  }
  return fd;
}
...
{
  mutex::scoped_lock descriptors_lock(registered_descriptors_mutex_);
  return registered_descriptors_.alloc(MXASIO_CONCURRENCY_HINT_IS_LOCKING(
...
#undef MXASIO_KQUEUE_EV_SET

#include "mxasio/detail/pop_options.hpp"

#endif // defined(MXASIO_HAS_KQUEUE)

#endif // MXASIO_DETAIL_IMPL_KQUEUE_REACTOR_IPP
