...
#ifndef MXASIO_DETAIL_SERVICE_REGISTRY_HPP
#define MXASIO_DETAIL_SERVICE_REGISTRY_HPP
...
public:
  // Constructor.
  MXASIO_DECL service_registry(execution_context& owner);

  // Destructor.
  MXASIO_DECL ~service_registry();

  // Shutdown all services.
  MXASIO_DECL void shutdown_services();

  // Destroy all services.
  MXASIO_DECL void destroy_services();
...
private:
  ...
  MXASIO_DECL static void init_key_from_id(
  ...
  MXASIO_DECL static bool keys_match(
  ...
  MXASIO_DECL static void destroy(execution_context::service* service);
  ...
  // create a new service object automatically if no such object already
  // exists. Ownership of the service object is not transferred to the caller.
  MXASIO_DECL execution_context::service* do_use_service(
      const execution_context::service::key& key,
      factory_type factory, void* owner);

  // Add a service object. Throws on error, in which case ownership of the
  // object is retained by the caller.
  MXASIO_DECL void do_add_service(
      const execution_context::service::key& key,
      execution_context::service* new_service);

  // Check whether a service object with the specified key already exists.
  MXASIO_DECL bool do_has_service(
      const execution_context::service::key& key) const;

  // Mutex to protect access to internal data.
  mutable mxasio::detail::mutex mutex_;
