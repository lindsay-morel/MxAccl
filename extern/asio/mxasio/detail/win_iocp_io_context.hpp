...
#ifndef MXASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
#define MXASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
...
#include "mxasio/detail/win_iocp_thread_info.hpp"
...
public:
  ...
  MXASIO_DECL win_iocp_io_context(mxasio::execution_context& ctx,
      int concurrency_hint = -1, bool own_thread = true);

  // Destructor.
  MXASIO_DECL ~win_iocp_io_context();
  ...
  MXASIO_DECL mxasio::error_code register_handle(
      HANDLE handle, mxasio::error_code& ec);

  // Run the event loop until stopped or no more work.
  MXASIO_DECL size_t run(mxasio::error_code& ec);

  // Run until stopped or one operation is performed.
  MXASIO_DECL size_t run_one(mxasio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  MXASIO_DECL size_t wait_one(long usec, mxasio::error_code& ec);

  // Poll for operations without blocking.
  MXASIO_DECL size_t poll(mxasio::error_code& ec);

  // Poll for one operation without blocking.
  MXASIO_DECL size_t poll_one(mxasio::error_code& ec);
  ...
  MXASIO_DECL void post_deferred_completion(win_iocp_operation* op);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operations.
  MXASIO_DECL void post_deferred_completions(
  ...
  MXASIO_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  MXASIO_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  MXASIO_DECL void on_completion(win_iocp_operation* op,
      const mxasio::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  MXASIO_DECL size_t do_one(DWORD msec,
      win_iocp_thread_info& this_thread, mxasio::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  MXASIO_DECL static DWORD get_gqcs_timeout();
...
#include "mxasio/detail/impl/win_iocp_io_context.hpp"
...
#endif // MXASIO_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
