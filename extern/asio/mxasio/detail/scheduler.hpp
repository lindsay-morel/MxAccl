...
#ifndef MXASIO_DETAIL_SCHEDULER_HPP
#define MXASIO_DETAIL_SCHEDULER_HPP
...
#include "mxasio/detail/conditionally_enabled_event.hpp"
#include "mxasio/detail/conditionally_enabled_mutex.hpp"
...
#include "mxasio/detail/thread_context.hpp"
...
public:
  ...
  MXASIO_DECL std::size_t run(mxasio::error_code& ec);

  // Run until interrupted or one operation is performed.
  MXASIO_DECL std::size_t run_one(mxasio::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  MXASIO_DECL std::size_t wait_one(
      long usec, mxasio::error_code& ec);

  // Poll for operations without blocking.
  MXASIO_DECL std::size_t poll(mxasio::error_code& ec);

  // Poll for one operation without blocking.
  MXASIO_DECL std::size_t poll_one(mxasio::error_code& ec);
  ...
  MXASIO_DECL void compensating_work_started();
  ...
  MXASIO_DECL bool can_dispatch();

  /// Capture the current exception so it can be rethrown from a run function.
  MXASIO_DECL void capture_current_exception();

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() has not yet been called for the operation.
  MXASIO_DECL void post_immediate_completion(
      operation* op, bool is_continuation);

  // Request invocation of the given operations and return immediately. Assumes
  // that work_started() has not yet been called for the operations.
  MXASIO_DECL void post_immediate_completions(std::size_t n,
      op_queue<operation>& ops, bool is_continuation);

  // Request invocation of the given operation and return immediately. Assumes
  // that work_started() was previously called for the operation.
  MXASIO_DECL void post_deferred_completion(operation* op);

  // Request invocation of the given operations and return immediately. Assumes
  // that work_started() was previously called for each operation.
  MXASIO_DECL void post_deferred_completions(op_queue<operation>& ops);

  // Enqueue the given operation following a failed attempt to dispatch the
  // operation for immediate invocation.
  MXASIO_DECL void do_dispatch(operation* op);
  ...
  MXASIO_DECL void abandon_operations(op_queue<operation>& ops);
...
private:
  ...
  MXASIO_DECL std::size_t do_run_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const mxasio::error_code& ec);

  // Run at most one operation with a timeout. May block.
  MXASIO_DECL std::size_t do_wait_one(mutex::scoped_lock& lock,
      thread_info& this_thread, long usec, const mxasio::error_code& ec);

  // Poll for at most one operation.
  MXASIO_DECL std::size_t do_poll_one(mutex::scoped_lock& lock,
      thread_info& this_thread, const mxasio::error_code& ec);

  // Stop the task and all idle threads.
  MXASIO_DECL void stop_all_threads(mutex::scoped_lock& lock);

  // Wake a single idle thread, or the task, and always unlock the mutex.
  MXASIO_DECL void wake_one_thread_and_unlock(
      mutex::scoped_lock& lock);

  // Get the default task.
  MXASIO_DECL static scheduler_task* get_default_task(
      mxasio::execution_context& ctx);
...
#endif // MXASIO_DETAIL_SCHEDULER_HPP
