...
#ifndef MXASIO_DETAIL_IO_URING_SERVICE_HPP
#define MXASIO_DETAIL_IO_URING_SERVICE_HPP
...
#include "mxasio/detail/object_pool.hpp"
...
public:
  ...
  class io_queue : operation
  {
    ...
    MXASIO_DECL io_queue();
    void set_result(int r) { task_result_ = static_cast<unsigned>(r); }
    MXASIO_DECL operation* perform_io(int result);
    MXASIO_DECL static void do_complete(void* owner, operation* base,
  ...
  class io_object
  {
    ...
    MXASIO_DECL io_object(bool locking);
  };
  ...
  // Constructor.
  MXASIO_DECL io_uring_service(mxasio::execution_context& ctx);

  // Destructor.
  MXASIO_DECL ~io_uring_service();
  ...
  MXASIO_DECL void register_io_object(io_object*& io_obj);

  // Register an internal I/O object with io_uring.
  MXASIO_DECL void register_internal_io_object(
      io_object*& io_obj, int op_type, io_uring_operation* op);

  // Register buffers with io_uring.
  MXASIO_DECL void register_buffers(const ::iovec* v, unsigned n);

  // Unregister buffers from io_uring.
  MXASIO_DECL void unregister_buffers();
  ...
  MXASIO_DECL void start_op(int op_type, per_io_object_data& io_obj,
  ...
  // error.
  MXASIO_DECL void cancel_ops(per_io_object_data& io_obj);
  ...
  MXASIO_DECL void cancel_ops_by_key(per_io_object_data& io_obj,
  ...
  MXASIO_DECL void deregister_io_object(per_io_object_data& io_obj);

  // Perform any post-deregistration cleanup tasks associated with the I/O
  // object.
  MXASIO_DECL void cleanup_io_object(per_io_object_data& io_obj);
...
private:
  ...
  // Initialise the ring.
  MXASIO_DECL void init_ring();

  // Register the eventfd descriptor for readiness notifications.
  MXASIO_DECL void register_with_reactor();

  // Allocate a new I/O object.
  MXASIO_DECL io_object* allocate_io_object();

  // Free an existing I/O object.
  MXASIO_DECL void free_io_object(io_object* s);
  ...
  MXASIO_DECL bool do_cancel_ops(
  ...
  MXASIO_DECL void update_timeout();

  // Get the current timeout value.
  MXASIO_DECL __kernel_timespec get_timeout() const;

  // Get a new submission queue entry, flushing the queue if necessary.
  MXASIO_DECL ::io_uring_sqe* get_sqe();

  // Submit pending submission queue entries.
  MXASIO_DECL void submit_sqes();

  // Post an operation to submit the pending submission queue entries.
  MXASIO_DECL void post_submit_sqes_op(mutex::scoped_lock& lock);

  // Push an operation to submit the pending submission queue entries.
  MXASIO_DECL void push_submit_sqes_op(op_queue<operation>& ops);
  ...
  class submit_sqes_op : operation
  {
    ...
    MXASIO_DECL submit_sqes_op(io_uring_service* s);
    MXASIO_DECL static void do_complete(void* owner, operation* base,
        const mxasio::error_code& ec, std::size_t bytes_transferred);
  };
...
#include "mxasio/detail/impl/io_uring_service.hpp"
...
#endif // MXASIO_DETAIL_IO_URING_SERVICE_HPP
