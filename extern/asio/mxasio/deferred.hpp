...
#ifndef MXASIO_DEFERRED_HPP
#define MXASIO_DEFERRED_HPP
...
public:
  ...
  template <MXASIO_COMPLETION_TOKEN_FOR(Signatures...) CompletionToken>
...
class MXASIO_NODISCARD deferred_values
{
private:
...
public:
  ...
  template <MXASIO_COMPLETION_TOKEN_FOR(void(Values...)) CompletionToken>
  auto operator()(CompletionToken&& token) &&
    -> decltype(
      this->invoke_helper(
...
class MXASIO_NODISCARD deferred_async_operation
{
private:
...
public:
  ...
  template <MXASIO_COMPLETION_TOKEN_FOR(Signature) CompletionToken>
  auto operator()(CompletionToken&& token) &&
    -> decltype(
      this->invoke_helper(
        static_cast<CompletionToken&&>(token),
...
template <typename Head, typename Tail>
class MXASIO_NODISCARD deferred_sequence :
...
class MXASIO_NODISCARD deferred_conditional
{
private:
 ...
 * operation, typically using the special value @c mxasio::deferred. For
 * example:
 *
 * @code auto my_deferred_op
 *   = my_socket.async_read_some(my_buffer,
 *       mxasio::deferred); @endcode
 *
...
/**
 * See the documentation for mxasio::deferred_t for a usage example.
 */
...
#include "mxasio/impl/deferred.hpp"

#endif // MXASIO_DEFERRED_HPP
