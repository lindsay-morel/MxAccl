...
#ifndef MXASIO_BUFFERED_READ_STREAM_HPP
#define MXASIO_BUFFERED_READ_STREAM_HPP
...
#include "mxasio/detail/buffer_resize_guard.hpp"
#include "mxasio/detail/buffered_stream_storage.hpp"
...
public:
...
#else
  MXASIO_STATIC_CONSTANT(std::size_t, default_buffer_size = 1024);
#endif
  ...
  std::size_t fill(mxasio::error_code& ec);
  ...
  /// Determine the amount of data that may be read without blocking.
  std::size_t in_avail(mxasio::error_code& ec)
  {
...
private:
  ...
  {
    std::size_t bytes_copied = mxasio::buffer_copy(
  ...
  {
    return mxasio::buffer_copy(buffers, storage_.data(), storage_.size());
  }
...
#include "mxasio/impl/buffered_read_stream.hpp"

#endif // MXASIO_BUFFERED_READ_STREAM_HPP
