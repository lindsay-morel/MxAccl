...
#ifndef MXASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
#define MXASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
...
#include "mxasio/local/detail/endpoint.hpp"
...
{
  if (new_size > sizeof(mxasio::detail::sockaddr_un_type))
  {
...
void endpoint::init(const char* path_name, std::size_t path_length)
{
  if (path_length > sizeof(data_.local.sun_path) - 1)
  {
    ...
    mxasio::error_code ec(mxasio::error::name_too_long);
    mxasio::detail::throw_error(ec);
  }

  using namespace std; // For memset and memcpy.
  memset(&data_.local, 0, sizeof(mxasio::detail::sockaddr_un_type));
...
#endif // MXASIO_LOCAL_DETAIL_IMPL_ENDPOINT_IPP
