...
#ifndef MXASIO_BASIC_SIGNAL_SET_HPP
#define MXASIO_BASIC_SIGNAL_SET_HPP
...
#include "mxasio/detail/signal_set_service.hpp"
 ...
 * mxasio::signal_set signals(my_context, SIGINT, SIGTERM);
 *
...
public:
   ...
   * @code mxasio::signal_set signals(ex);
    ...
    : impl_(0, ex)
  {
    ...
    mxasio::detail::throw_error(ec, "add");
  }
   ...
   *
   * @note This constructor is equivalent to performing:
   * @code mxasio::signal_set signals(context);
   * signals.add(signal_number_1); @endcode
   */
  template <typename ExecutionContext>
   ...
   */
  MXASIO_SYNC_OP_VOID add(int signal_number,
      mxasio::error_code& ec)
  {
    impl_.get_service().add(impl_.get_implementation(), signal_number, ec);
   ...
   * @param ec Set to indicate what error occurred, if any.
   */
  MXASIO_SYNC_OP_VOID add(int signal_number, flags_t f,
      mxasio::error_code& ec)
  {
    impl_.get_service().add(impl_.get_implementation(), signal_number, f, ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  ...
  void remove(int signal_number)
  {
    ...
    mxasio::detail::throw_error(ec, "remove");
  }
   ...
   * signal number.
   */
  MXASIO_SYNC_OP_VOID remove(int signal_number,
      mxasio::error_code& ec)
  {
    impl_.get_service().remove(impl_.get_implementation(), signal_number, ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  ...
  void clear()
  {
    ...
    mxasio::detail::throw_error(ec, "clear");
  }
   ...
   * @note Removes all queued notifications.
   */
  MXASIO_SYNC_OP_VOID clear(mxasio::error_code& ec)
  {
    impl_.get_service().clear(impl_.get_implementation(), ec);
    MXASIO_SYNC_OP_VOID_RETURN(ec);
  }
   ...
   * operation will be invoked with the mxasio::error::operation_aborted
   * error code.
   *
   ...
   * error code mxasio::error::operation_aborted.
   *
   ...
   * @code void(mxasio::error_code, int) @endcode
   *
   ...
   */
  template <
    MXASIO_COMPLETION_TOKEN_FOR(void (mxasio::error_code, int))
      SignalToken = default_completion_token_t<executor_type>>
  auto async_wait(
      SignalToken&& token = default_completion_token_t<executor_type>())
    -> decltype(
      async_initiate<SignalToken, void (mxasio::error_code, int)>(
        declval<initiate_async_wait>(), token))
  {
    return async_initiate<SignalToken, void (mxasio::error_code, int)>(
...
private:
  ...
  public:
    ...
    void operator()(SignalHandler&& handler) const
    {
      ...
      MXASIO_SIGNAL_HANDLER_CHECK(SignalHandler, handler) type_check;
...
#endif // MXASIO_BASIC_SIGNAL_SET_HPP
