...
#ifndef MXASIO_THREAD_HPP
#define MXASIO_THREAD_HPP
...
/**
 * The mxasio::thread class implements the smallest possible subset of the
 ...
 *
 * @par Example
 * A typical use of mxasio::thread would be to launch a thread to run an
 * io_context's event processing loop:
 *
 * @par
 * @code mxasio::io_context io_context;
 * // ...
 * mxasio::thread t(boost::bind(&mxasio::io_context::run, &io_context));
 * // ...
 * t.join(); @endcode
 */
class thread
