...
#ifndef MXASIO_EXECUTION_ANY_EXECUTOR_HPP
#define MXASIO_EXECUTION_ANY_EXECUTOR_HPP
...
#include "mxasio/detail/cstddef.hpp"
#include "mxasio/detail/executor_function.hpp"
...
public:
   ...
   * mxasio::require and mxasio::prefer customisation points.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = mxasio::requre(ex, execution::blocking.possibly); @endcode
   */
  ...
  /**
   * Do not call this function directly. It is intended for use with the
   * mxasio::prefer customisation point.
   *
   * For example:
   * @code execution::any_executor<execution::blocking_t::possibly_t> ex = ...;
   * auto ex2 = mxasio::prefer(ex, execution::blocking.possibly); @endcode
   */
  template <typename Property>
   ...
   * size_t n = mxasio::query(ex, execution::occupancy); @endcode
   */
  template <typename Property>
  ...
  /**
   * Throws mxasio::bad_executor if the polymorphic wrapper has no target.
   */
...
public:
  ...
  {
    if (impl_)
      mxasio::detail::ref_count_up(impl_->ref_count_);
  }
  ...
  {
    if (impl_)
      if (mxasio::detail::ref_count_down(impl_->ref_count_))
        delete impl_;
  }
...
private:
  ...
  struct impl_base
  {
    ...
    virtual ~impl_base() {}
    virtual void* get() = 0;
    mxasio::detail::atomic_count ref_count_;
  };
...
public:
  ...
  template <MXASIO_EXECUTION_EXECUTOR Executor>
  ...
  void execute(F&& f) const
  {
    if (target_)
    {
      ...
      {
        mxasio::detail::non_const_lvalue<F> f2(f);
...
protected:
  ...
  typedef mxasio::detail::executor_function function;
  typedef mxasio::detail::executor_function_view function_view;
...
# pragma warning (disable:4702)
  ...
  static void query_fn_non_void(void*, const void* ex, const void* prop,
      enable_if_t<
        mxasio::can_query<const Ex&, const Prop&>::value
          && is_same<typename Prop::polymorphic_query_result_type, void>::value
      >*)
  {
    mxasio::query(*static_cast<const Ex*>(ex),
  ...
  static Poly require_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return mxasio::require(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
  ...
  static Poly prefer_fn_impl(const void* ex, const void* prop,
      ...
      >*)
  {
    return mxasio::prefer(*static_cast<const Ex*>(ex),
        *static_cast<const Prop*>(prop));
  }
...
private:
  ...
  {
    return mxasio::query(ex, execution::blocking);
  }
...
//  template <typename...> friend class any_executor;
  ...
  typedef aligned_storage<
      sizeof(mxasio::detail::shared_ptr<void>) + sizeof(void*),
      alignment_of<mxasio::detail::shared_ptr<void>>::value
    >::type object_type;
...
public:
      ...
      >* = 0) const
  {
    ...
    prop_fns_[found::index].query(&result, object_fns_->target(*this),
        &static_cast<const typename found::type&>(p));
    return *mxasio::detail::scoped_ptr<
      typename found::query_result_type>(result);
  }
...
#endif // MXASIO_EXECUTION_ANY_EXECUTOR_HPP
