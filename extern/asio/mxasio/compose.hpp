...
#ifndef MXASIO_COMPOSE_HPP
#define MXASIO_COMPOSE_HPP
 ...
 *       {
 *         state_ = writing;
 *         mxasio::async_write(socket_, buffer_,
 *             mxasio::transfer_exactly(n),
 *             std::move(self));
 *       }
 *       break;
 *     case writing:
 ...
 *   decltype(
 *     mxasio::async_compose<CompletionToken,
 *       void(mxasio::error_code, std::size_t)>(
 *         std::declval<async_echo_implementation>(),
 *         token, socket))
 * {
 *   return mxasio::async_compose<CompletionToken,
 *     void(mxasio::error_code, std::size_t)>(
